...
- **Soft-delete**: Moves to trash (PATCH sets deletedAt). Can be restored or permanently deleted
...
Note: The admin fetches are routed through the admin proxy at `/api/v1/admin/settings/checkout-languages`, which maps to the API's `/checkout-languages` routes.
