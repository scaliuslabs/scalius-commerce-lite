...
**Navigation Performance**: The sidebar intent-preloads data only for the seven principal read-only list routes whose loaders and rendered components share exact Query keys. After hydration, browser-idle work warms only permission-visible route code with `router.loadRouteChunk()`; it never runs route loaders or API reads, is capped at two route chains (one on 3G), and pauses for Save Data, 2G, offline, hidden, and page-hide states. The router keeps the current useful screen mounted throughout navigation and shows a non-blocking top progress rail only after 180 ms; cold lazy pages and settings panes use stable page-shaped skeletons rather than generic spinners. `useServerTable()` suppresses only the immediate duplicate within a five-second intent-prefetch grace period; invalidated data and ordinary route returns must revalidate even while the domain query remains nominally fresh. Do not speculatively warm adjacent Orders or Customers pages or bulk-sync protected surface data: abandoned work must not amplify reads or retain extra buyer PII in the client cache. Keep one-second clocks inside their smallest rendered control, keep persistent shell location subscriptions selected to pathname, and preserve the memoized desktop row boundary. Route search validation must import lightweight discriminator/state leaves rather than pulling full editor models into the global registry. See [the measured release audit](../../docs/PERFORMANCE-RELEASE.md) before changing router pending behavior, adopting Table v9, adding virtualization, or broadening preload behavior.

**Client Chunk Safety**: Keep the framework's `$initial` Rolldown group unsplit. A size cap can divide a strongly connected ESM graph and crash hydration before login becomes interactive. Every production admin build rejects reciprocal static chunk imports; preserve that gate when changing bundle grouping.
...
**Order Fulfillment**: Order detail supports provider shipments and own-courier/manual fulfillment. `ManualFulfillmentDialog` posts selected unshipped item IDs to the typed orders server-function slice, invalidates order detail + shipments, and computes final-shipment intent from the remaining fulfillable items. Manual shipment history rows can show courier/tracking/note metadata, but the refresh action is only shown for provider-backed shipments. If a provider shipment is created but local finalization needs repair, the shipment recovery notice shows sanitized copy and an RBAC-gated `Repair shipment` action for shipment managers; the action reconciles local order/inventory state from persisted shipment evidence and does not create another provider shipment.
...
Checkout Flow uses its existing field-wise rebase for save acknowledgments and revision conflicts. It pauses background synchronization during saves and conflict resolution, preserving explicit reverts as well as newer edits. After a write failure, a newer authoritative read can advance the saved baseline without replacing those edits. Each subsequent save uses the accepted server revision.
