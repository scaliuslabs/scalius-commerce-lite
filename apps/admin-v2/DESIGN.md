...
| Shell | `bg-sidebar` (the near-black navigation frame, both themes) `text-sidebar-foreground` `text-sidebar-muted-foreground` `hover:bg-sidebar-hover` `bg-sidebar-accent` (active pill, search field hover) `bg-store-badge` `text-store-badge-foreground` (store avatar) · `bg-topbar` `text-topbar-foreground` (save bar, toasts) |
...
Choosing one option: a short fixed choice (a status, a sort, a handful of enum values written in code) is a `NativeSelect`, a styled native `<select>` as in Polaris, with the platform picker on phones and no scroll quirks; options are plain text. Anything whose options come from data (customers, products, categories, collections, pages, staff, couriers, discounts, places, groups) or can pass about ten entries is a `SearchableSelect`: static `options` for a list already loaded, or `load` + `queryKey` for a large source searched on the server and paged (debounced; the search never goes in the page URL), with `description` for secondary text ("Mirpur-2 · Dhaka"), `group` headings and `clearable`. Delivery addresses always use `LocationPicker` (City → Thana → Area, each level filtered by the one above). The Radix Select is gone: lint rejects `@radix-ui/react-select`, `components/ui/select` and a raw `<select>` outside `NativeSelect`.
