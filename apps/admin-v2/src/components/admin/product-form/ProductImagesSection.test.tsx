...
function media(id: string, name: string, sortOrder: number, kind: "image" | "video" = "image"): ProductMediaItem {
  return {
    ...
    effectiveAltText: name, altText: "", caption: null, width: 10, height: 10, durationMs: null,
...
function Harness() {
  ...
  return <Form {...form}><ProductImagesSection form={form} /></Form>;
}
...
describe("ProductImagesSection", () => {
  ...
  it("makes a photo the main one and reorders without dragging", () => {
    ...
    expect(form.getValues("media").map((item) => item.effectiveAltText)).toEqual(["blue.jpg", "red.jpg", "demo.mp4"]);
  });
  ...
  it("edits one item's description in a labelled field", () => {
    ...
    expect(input!.placeholder).toBe("blue.jpg");
  });
});
