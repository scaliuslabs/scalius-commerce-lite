...
export function useCustomerListActions({
  ...
  const fetchCustomers = useCallback(
    async (params: {
      ...
      try {
        ...
        }) as { customers: Record<string, unknown>[]; pagination: CustomerListPagination };
    ...
    async (
      ...
      try {
        ...
        toast.error(errorTitle, {
          description: getServerFnError(error, "An unknown error occurred."),
        });
      } finally {
  ...
  }, [performServerAction, setDisplayCustomers, setCurrentPagination]);

  const handlePermanentDelete = useCallback((id: string) => {
    performServerAction(
      () => permanentDeleteCustomer({ data: { id } }),
      {
  ...
  const handleBulkAction = useCallback(() => {
    ...
    performServerAction(
      () => bulkDeleteCustomers({ data: { customerIds: ids, permanent: showTrashed } }),
      {
  ...
  }, [selectedCustomers, showTrashed, performServerAction, setDisplayCustomers, setCurrentPagination]);
