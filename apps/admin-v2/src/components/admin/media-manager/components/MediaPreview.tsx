      ...
      <DialogContent className="flex h-[92svh] max-h-[760px] w-[96vw] max-w-5xl flex-col overflow-hidden p-0 sm:h-[88vh] sm:w-[94vw]">
        ...
        <div className="flex min-h-0 flex-1 flex-col md:grid md:grid-cols-[minmax(0,1fr)_18rem]">
          <div className="relative flex min-h-0 flex-[0_0_15rem] items-center justify-center overflow-hidden bg-muted/50 p-3 md:flex-auto">
            ...
            <Button type="button" variant="secondary" size="icon" className="absolute left-3 top-1/2 h-9 w-9 -translate-y-1/2 sm:h-8 sm:w-8" disabled={index <= 0} onClick={() => requestNavigation(-1)} aria-label="Previous asset"><ChevronLeft className="h-4 w-4" /></Button>
            <Button type="button" variant="secondary" size="icon" className="absolute right-3 top-1/2 h-9 w-9 -translate-y-1/2 sm:h-8 sm:w-8" disabled={index < 0 || index >= files.length - 1} onClick={() => requestNavigation(1)} aria-label="Next asset"><ChevronRight className="h-4 w-4" /></Button>
          </div>
          <aside className="min-h-0 overflow-y-auto border-t p-4 md:border-l md:border-t-0">
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button type="button" variant="outline" size="sm" className="h-9 flex-1 sm:h-8" onClick={async () => { try { await navigator.clipboard.writeText(sourceUrl); toast.success("URL copied"); } catch { toast.error("URL could not be copied", { description: "Copy it from the opened original instead." }); } }}><Copy className="mr-1.5 h-3.5 w-3.5" />Copy URL</Button>
                <Button type="button" variant="outline" size="sm" className="h-9 flex-1 sm:h-8" asChild><a href={sourceUrl} download={file.filename} target="_blank" rel="noreferrer"><Download className="mr-1.5 h-3.5 w-3.5" />Download</a></Button>
              </div>
            </div>
          </aside>
        </div>
        <DialogFooter className="border-t px-4 py-3 sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">{dirty && <Button type="button" variant="ghost" size="sm" onClick={resetDraft}><RotateCcw className="mr-1.5 h-3.5 w-3.5" />Reset</Button>}{onSelect && <Button type="button" variant="outline" size="sm" onClick={() => onSelect(file)}>Use this {file.kind}</Button>}<Button type="button" size="sm" disabled={!filename.trim() || !dirty || saving} onClick={() => void save()}>{saving && <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />}Save details</Button></div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
