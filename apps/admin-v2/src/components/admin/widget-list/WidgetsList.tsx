...
import { getOpenRouterSettings, updateOpenRouterSettings } from "@/lib/api.functions";
...
export function WidgetsList({
    ...
    if (isSettingsOpen) {
      getOpenRouterSettings()
        .then((data: Record<string, unknown>) => {
          setOpenRouterApiKey((data.apiKey as string) || "");
        })
        .catch(() => {});
    }
  }, [isSettingsOpen]);
  ...
  const handleSaveApiKey = async () => {
    setIsSavingKey(true);
    try {
      await updateOpenRouterSettings({ data: { apiKey: openRouterApiKey } });
  ...
  const handleCreateClick = () => {
    void navigate({ to: "/admin/widgets/create" as string });
  };
