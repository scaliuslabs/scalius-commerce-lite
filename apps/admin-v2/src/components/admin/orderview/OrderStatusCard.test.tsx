...
const order = {
  id: "ord_1001", version: 1, status: "pending", paymentStatus: "unpaid", paidAmount: 0,
} as Order;
...
describe("OrderStatusCard", () => {
  ...
  async function chooseStatus(label: string) {
    const trigger = host.querySelector<HTMLButtonElement>('[aria-label="Order status"]');
    await act(async () => {
      trigger?.focus();
  ...
  it("asks before cancelling and only cancels after confirmation", async () => {
    ...
    expect(dialog?.textContent).toContain("Cancel order #ord_1001?");
    expect(dialog?.textContent).toContain("Stock is released and the customer is notified.");
    expect(mocks.mutate).not.toHaveBeenCalled();

    const keep = [...dialog!.querySelectorAll("button")].find((button) => button.textContent === "Keep order");
    await act(async () => keep?.click());
    expect(mocks.mutate).not.toHaveBeenCalled();

    await chooseStatus("Cancelled");
    const confirm = [...document.querySelectorAll('[role="alertdialog"] button')]
      .find((button) => button.textContent === "Cancel order") as HTMLButtonElement | undefined;
