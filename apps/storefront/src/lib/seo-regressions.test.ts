...
const storefrontRoot = [cwd(), join(cwd(), "apps/storefront")].find(
  (candidate) => existsSync(join(candidate, "src/pages/cart.astro")),
);

if (!storefrontRoot) {
  throw new Error(
    "Unable to locate storefront package root for SEO regression tests",
  );
}
...
describe("storefront SEO regressions", () => {
  it("does not publish cart URLs in the static sitemap", async () => {
    const source = await readFile(
      join(storefrontRoot, "src/pages/sitemap-static.xml.ts"),
      "utf8",
    );
  ...
  it("marks the cart page noindex", async () => {
    const source = await readFile(
      join(storefrontRoot, "src/pages/cart.astro"),
      "utf8",
    );
  ...
  it("keeps global OnlineStore JSON-LD behind absolute storefront and logo URLs", async () => {
    ...
    expect(source).toMatch(
      /const websiteJsonLd =\s+discoverySettings\.structuredData\.websiteSearch &&\s+storefrontUrl &&\s+storeSchemaName/,
    );
    ...
    expect(source).not.toContain(
      'logo: { "@type": "ImageObject", url: getOptimizedImageUrl',
    );
  });
  ...
  it("normalizes Open Graph and Twitter images to absolute storefront URLs", async () => {
    ...
    expect(source).toContain(
      "const absoluteOgImageUrl = toAbsoluteStorefrontSeoUrl(ogImage)",
    );
    expect(source).toContain(
  ...
  it("keeps sitemap-advertised search canonical and noindexes listing variants", async () => {
    const source = await readFile(
      join(storefrontRoot, "src/pages/search/index.astro"),
      "utf8",
    );
