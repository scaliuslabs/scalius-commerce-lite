...
describe("checkout validate-cart proxy", () => {
  ...
  it("rejects synthetic default variant ids before backend validation", async () => {
    const response = await POST({
      request: new Request("https://storefront.example.test/api/checkout/validate-cart", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    expect(await response.json()).toMatchObject({
      success: false,
      error: "Every cart item must include a saved product variant.",
    });
    expect(mocks.validateCartItems).not.toHaveBeenCalled();
  });

  it("rejects the whole payload when any submitted line lacks a persisted variant", async () => {
    const response = await POST({
      request: new Request("https://storefront.example.test/api/checkout/validate-cart", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          items: [
            { productId: "prod_1", variantId: "var_1", quantity: 1, price: 150 },
            { productId: "prod_2", quantity: 1, price: 200 },
          ],
        }),
      }),
    } as never);
  ...
  it("preserves structured validation failure details and status", async () => {
    ...
    const response = await POST({
      request: new Request("https://storefront.example.test/api/checkout/validate-cart", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ items: [{ productId: "prod_1", variantId: "var_1", quantity: 1, price: 150 }] }),
      }),
    } as never);
