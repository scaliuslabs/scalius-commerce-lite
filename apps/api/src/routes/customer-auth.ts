...
// import { sendEmail } from "@scalius/core/integrations/email"; - Removed, now handled by queue consumer
...
app.post("/send-otp", async (c) => {
  try {
    ...
    // Push payload to Cloudflare Queue for async delivery (prevents 504 timeouts on flash sales)
    await c.env.AUTH_OTP_QUEUE.send({
      type: "auth.send_otp",
      method,
      allowedMethod,
      identifier,
...
app.post("/logout", async (c) => {
  ...
  // Domain-scoped clears (catch cookies set with a domain attribute):
  if (domainAttr) {
...
app.get("/orders", async (c) => {
  try {
    ...
    const db = c.get("db");
    const { orders, orderItems, products, productVariants, productImages } = await import("@scalius/database/schema");
