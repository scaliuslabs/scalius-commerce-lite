...
import {
  ...
  describeWidgetCompositionPlan,
} from "@scalius/core/modules/ai/destination-contracts";
import {
  GENERATION_CONFIG,
  getTimeout,
  getWidgetAiPrompt,
...
import type {
  WidgetDesignAgentState,
  WidgetDesignAgentStoredEvent,
...
const selectedImageSchema = z
  .object({
    ...
    url: z
      ...
      .refine(
        isAllowedWidgetImageUrl,
        "Image URLs must use HTTPS or data URLs.",
      ),
...
const widgetGenerationRunSchema = z.object({
  ...
  productIds: z
    ...
    .max(GENERATION_CONFIG.context.maxProducts)
    .optional(),
  categoryIds: z
    .array(z.string().max(200))
    .max(GENERATION_CONFIG.context.maxCategories)
    .optional(),
  collectionIds: z
    .array(z.string().max(200))
    .max(GENERATION_CONFIG.context.maxCollections)
    .optional(),
  anchorCollectionIds: z
    .array(z.string().max(200))
    .max(GENERATION_CONFIG.context.maxCollections)
    .optional(),
...
function messageFromError(error: unknown): string {
  return error instanceof Error
    ? error.message
    : String(error ?? "Widget generation failed");
}
...
function getImprovementCode(payload: WidgetGenerationRunPayload): {
  html: string;
  css: string;
  js: string;
} {
  if (
    payload.targetSection !== undefined &&
    payload.sections?.[payload.targetSection]
  ) {
...
function buildPreviewPatchFromRaw(
  rawText: string,
  commerceFactsProvided: boolean,
): { html: string; css: string; js?: string } | null {
  try {
    const normalized = normalizeWidgetGenerationText(rawText, {
...
function parseNormalizedArtifact(text: string): {
  html: string;
  css: string;
  js: string;
} {
...
function buildPreviewObservation(
  artifacts: Array<{ html: string; css: string; js: string }>,
): string {
  ...
  const hasResponsiveCss =
    /@media|clamp\(|minmax\(|grid-template|flex-wrap/i.test(mergedCss);
...
function buildSectionGenerationPrompt({
  ...
  const workspace = buildArtifactWorkspace(
    previousArtifacts,
    buildPreviewObservation(previousArtifacts),
  );
...
function sumUsage(
  results: WidgetGenerationResult[],
): WidgetGenerationResult["usage"] {
  return results.reduce<WidgetGenerationResult["usage"]>(
    (usage, result) => ({
      inputTokens:
        (usage.inputTokens ?? 0) + (result.usage.inputTokens ?? 0) || undefined,
      outputTokens:
        (usage.outputTokens ?? 0) + (result.usage.outputTokens ?? 0) ||
        undefined,
      totalTokens:
        (usage.totalTokens ?? 0) + (result.usage.totalTokens ?? 0) || undefined,
    }),
    {},
  );
}
...
function productImageUrl(product: Record<string, unknown>): {
  ...
  const imageUrl = readString(product, [
    "imageUrl",
    "image",
    "thumbnail",
    "thumbnailUrl",
  ...
  if (imageUrl)
    return {
      ...
      alt: readString(
        product,
        ["imageAlt", "alt"],
        readString(product, ["name"], "Product image"),
      ),
    };
  ...
  if (!primary)
    return { url: "", alt: readString(product, ["name"], "Product image") };
  return {
...
function renderDeterministicProductCard(
...
function buildDeterministicSectionArtifact({
  ...
  const titleSubject =
    primaryCategory?.name || products[0]?.categoryName || "Featured picks";
  ...
  const sectionLabel =
    sectionIndex === 0
      ? `${titleSubject} homepage feature`
      : `${titleSubject} product picks`;
...
type WidgetDesignAgentRuntimeHost = {
  env: Env;
  request: Request;
  recordEvent: (runId: string, event: WidgetDesignAgentStoredEvent) => void;
  updateRunState: (next: Partial<WidgetDesignAgentState>) => void;
};

export async function streamWidgetDesignAgentRun(
  host: WidgetDesignAgentRuntimeHost,
): Promise<Response> {
  const { request } = host;
  const payload = widgetGenerationRunSchema.parse(await request.json());
  const db = getDb(host.env);
      ...
      try {
        ...
        const settings = await tools.run("load_settings", () =>
          getWidgetAiRuntimeSettings(
            db,
            host.env,
            getCredentialEncryptionKey(host.env),
          ),
        );
        const provider = (payload.provider ||
          settings.activeProvider) as WidgetAiProvider;
          ...
          () =>
            resolveAiContextBatchDetails({
              ...
              payload: {
                ...
                categoryIds: payload.allCategoriesSelected
                  ? undefined
                  : payload.categoryIds,
        ...
        host.updateRunState({ phase: "prompting" });
        const promptResult = await tools.run(
          "build_prompt",
          async () => {
            const systemPrompt = await getWidgetAiPrompt(
              db,
              payload.promptType,
            );
            const improvementCode =
              payload.operation === "improve"
                ? getImprovementCode(payload)
                : null;
            const prompt = await generateStructuredPrompt({
              systemPrompt,
              userPrompt:
                payload.operation === "create" ? payload.userPrompt : undefined,
              improvementPrompt:
                payload.operation === "improve"
                  ? buildImprovementInstruction(payload)
                  : undefined,
        ...
        host.updateRunState({ phase: "generating" });
        ...
        const result =
          payload.operation === "create"
            ? await (async () => {
                const plan = await tools.run(
                  "plan_artifact",
                  async () => createWidgetCompositionPlan(payload.promptType),
                  (value) => ({
                ...
                try {
                  for (
                    let index = 0;
                    index < plan.sectionDescriptions.length;
                    index++
                  ) {
                      ...
                      () =>
                        generateStructuredPrompt({
                          ...
                          allCategoriesSelected:
                            payload.allCategoriesSelected === true,
                          modelId,
                          supportsVision: capabilities.supportsVisionInput,
                          maxImagesOverride: capabilities.maxImages,
                          promptType: payload.promptType,
                      ...
                      async () => {
                        const generationOptions = {
                          ...
                          maxOutputTokens: Math.min(
                            ...
                            payload.promptType === "landing-page" ? 2600 : 2200,
                          ),
                        ...
                        try {
                          ...
                          for await (const delta of generation.textStream) {
                            ...
                            if (
                              rawText.length - lastPreviewPatchLength >=
                              600
                            ) {
                              const patch = buildPreviewPatchFromRaw(
                                rawText,
                                commerceFactsProvided,
                              );
                              if (patch) {
                          ...
                          if (!rawText.trim()) {
                            throw new Error(
                              "The AI provider returned an empty section artifact.",
                            );
                          }
                          const normalized = normalizeWidgetGenerationText(
                            rawText,
                            {
                              commerceFactsProvided,
                            },
                    ...
                    sectionArtifacts.push(
                      parseNormalizedArtifact(sectionResult.text),
                    );
                    const partial = parseNormalizedArtifact(
                      mergeSectionArtifacts(
                        sectionArtifacts,
                        payload.promptType,
                        commerceFactsProvided,
                      ),
                  ...
                  const fallbackText = mergeSectionArtifacts(
                    fallbackArtifacts,
                    payload.promptType,
                    true,
                  );
            ...
            : await tools.run("generate", async () => {
                ...
                try {
                  ...
                  if (!rawText.trim()) {
                    throw new Error(
                      "The AI provider returned an empty improved artifact.",
                    );
                  }
                  const normalized = normalizeWidgetGenerationText(rawText, {
                    commerceFactsProvided,
        ...
        host.updateRunState({ phase: "validated", artifactReady: true });
        emit({
        ...
        host.updateRunState({ phase: "complete", artifactReady: true });
        ...
        if (!isExpectedAbort(error, request.signal)) {
          ...
          emit({
            type: "run.failed",
            runId,
            error: { message: messageFromError(error) },
          });
        }
      } finally {
        if (!streamClosed) {
