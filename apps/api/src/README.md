...
| `adminAuthMiddleware` | `/admin/*`, `/cache/*` | Locally HMAC-verifies the Better Auth dashboard session cookie, reads the live session/user row from D1, and also accepts scanner session cookies only for exact scanner workflow endpoints. Then invited-admin onboarding, 2FA, and RBAC permission checks. |
...
1. **Better Auth session cookie** -- from the admin dashboard SSR frontend. The middleware verifies the signed `token.signature` cookie format with `BETTER_AUTH_SECRET` before D1; raw, tampered, wrong-secret, or unsigned cookies fail fast as unauthenticated and never call the Better Auth handler.
...
The helper deletes exact API product detail keys, query-varied product detail keys, product-search keys, and `api:search:*`, then enqueues storefront exact data keys (`product_slug_${slug}`, `product_variants_${productId}`) plus exact product HTML paths (`/products/${slug}`) with `bumpVersion: false` on the durable `storefront-cache` queue. When a changed product slug appears in a published CMS page shortcode, it also purges that page's exact API page-render pattern, `page_render_${slug}_` data generation, and CMS HTML path; if the affected page count exceeds `MAX_STOREFRONT_EXACT_HTML_PATHS`, the purge bumps the global HTML version instead of silently leaving excess pages stale. Exact storefront HTML paths are relative-only, canonicalized with the shared HTML cache-path rules before dedupe/capping, and capped at `MAX_STOREFRONT_EXACT_HTML_PATHS` (`20`) before the purge request is sent. Current call sites cover admin order create/edit/delete/restore/permanent-delete, bulk/manual shipment/status/COD/fulfillment/refund/return paths, payment queue mutations, delivery webhook reconciliation, admin shipment refresh, admin inventory variant adjustments (`/{variantId}/adjust`, `/stock-adjust`, `/stock-set`), the scheduled bounded orphan-reservation sweep, scheduled refund reconciliation, and scheduled stale hosted-payment order archival. Scheduled sweeps log `limit` and `hasMore`; a `hasMore=true` result means the next cron run should continue draining the backlog rather than widening a single Worker invocation.
...
Normal buyer checkout is synchronous and D1-fenced. `POST /orders` requires a stable `checkoutRequestId`, builds the canonical attempt identity, does a read-only `checkout_attempts` precheck so committed/active same-key retries return before mutable checkout policy or rate-limit gates, then claims new or reclaimable attempts before order writes. The claim reserves the canonical `orderId` and checkout/receipt token, the API commits the order before returning `201`, and the committed response is stored on the attempt row. Same-request retries replay committed responses or return a pollable `202` while the first request is active; exact duplicate active retries do not attempt a reclaim update after the first selected fresh row; changed payloads for the same key return `409`. Checkout-status and receipt KV rows are fast hints, while `/orders/status/:token` and receipt-token validation can fall back to D1 `checkout_attempts` plus the committed `orders` row. Status fallbacks that prove completion or failure schedule KV repair so later polls return to the fast path after transient KV misses.
...
| `payment.polar.refunded` | `processPolarWebhookRefund()` | Update payment status, release inventory on full refund, then enqueue a durable buyer refund notification when local processing succeeds (can originate from Polar dashboard) |
...
| `auth.send_otp` | Email / WhatsApp / SMS | Claim `auth_otp_delivery_receipts`, skip terminal/expired OTP attempts, terminalize merchant-actionable provider setup failures, then send via email (`sendEmail()` with Cloudflare Email Service default and Resend fallback), WhatsApp (Meta Graph API template), or SMS (`getActiveSmsProvider()` with 4 providers). `purpose: "order_payment_recovery"` uses payment-recovery copy for buyer hosted-payment recovery without creating customer sessions or exposing receipt proof. Resend receives `deliveryKey` as `Idempotency-Key`; GenNet receives a deterministic receipt-derived `csms_id`. |
| `auth-otp-dlq` | `markAuthOtpDeliveryReceiptSkippedByDeliveryKey()` | Archive terminal OTP queue failures by creating/updating the D1 receipt as skipped with redacted evidence; never calls email/SMS/WhatsApp providers. |
...
| `src/middleware/admin-auth.ts` | Admin auth (direct signed Better Auth session-cookie verification + scanner session cookie) + invited-admin onboarding gate + 2FA gate + RBAC |
