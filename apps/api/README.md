...
Every API HTTP response includes `X-Request-Id`. The worker preserves a safe incoming `X-Request-Id` or generates one, and structured ops logs include that request id plus Cloudflare `CF-Ray` when present. Degraded readiness emits an `api.readyz.degraded` ops log with request correlation and required-check statuses only.

For alert signals, read-only production smokes, queue/DLQ checks, and deploy rollback investigation pointers, see `../../audit/OPERATIONAL_RUNBOOK.md`.
...
Messages process independently with bounded all-settled concurrency. Successful messages are acked; generic failures retry with a 30-second delay, while auth OTP receipt-busy/transient failures can request the bounded D1 receipt backoff delay.
