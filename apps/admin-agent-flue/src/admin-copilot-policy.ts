...
export const ADMIN_ENTRY_PAGES: readonly AdminEntryPage[] = Object.freeze([
  ...
  { label: "New product", route: "/admin/products/new", aliases: ["Create product"] },
  { label: "Categories", route: "/admin/categories" },
  { label: "New category", route: "/admin/categories/new", aliases: ["Create category"] },
  { label: "Attributes", route: "/admin/attributes" },
  { label: "Collections", route: "/admin/collections" },
  { label: "New collection", route: "/admin/collections/new", aliases: ["Create collection"] },
  ...
  { label: "New order", route: "/admin/orders/new", aliases: ["Create order"] },
  {
  ...
  { label: "New customer", route: "/admin/customers/new", aliases: ["Create customer"] },
  { label: "Discounts", route: "/admin/discounts" },
  {
    label: "New discount",
    route: "/admin/discounts/new",
    aliases: ["Create discount", "Create promotion"],
  },
...
export function buildAdminCopilotInstructions(): string {
  ...
  return [
    ...
    "TASK NAVIGATION — If the latest message explicitly asks you to create a product, category, collection, order, customer, discount, or promotion, immediately open its exact `New …` route below as the first step. The task request itself authorizes that fixed application route; do not demand a separate navigation request and do not substitute a different route.",
    ...
    "BOUNDS — Use no more than four capability calls in one request. Never use shell, filesystem, local image generation, or hidden workspace tools. If the bounded capabilities cannot finish the task, state the exact missing fact or unavailable capability once and stop.",
    "Examples: `Take me to Products page` => immediately call computer with `goto /admin/products`; `Create a test product with real images` => immediately call computer with `goto /admin/products/new`; `Can you open Taxes?` => immediately call computer with `goto /admin/settings/taxes`; `How many products do we have?` => immediately call scalius with `call admin.api.get.products.stats -- {}`; `What am I looking at?` => call computer with `observe`.",
  ].join("\n");
}
