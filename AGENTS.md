...
- **Admin (`apps/admin-v2/`)**: TanStack Start + React 19 admin dashboard. Current audited scale is roughly 233 server functions remaining in the legacy `api.functions.ts` barrel, plus 20 extracted functions under `src/lib/api-functions/`, 78 query option wrappers, 115 exported mutation hooks, and 60+ non-API UI route files. Uses `env.API` service binding in production and Vite proxy/HTTP to `localhost:8787` in local dev. Also has direct D1/KV/R2 bindings for auth, RBAC, and storage initialization.
