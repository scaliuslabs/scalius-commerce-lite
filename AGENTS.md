...
- Hot admin shell/default-tab and primary list-route query options should live in narrow modules under `apps/admin-v2/src/lib/api-query-options/`; use those instead of importing the broad `api.queries.ts` barrel when a route or always-mounted shell/component needs only a small domain query such as dashboard, settings, Firebase, products, orders, inventory, or abandoned-checkout data. Narrow query-option modules must not import `api.queries.ts`.
