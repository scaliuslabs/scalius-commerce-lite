...
- Visible admin rich-text fields should render `DeferredTiptapEditor` from `apps/admin-v2/src/components/ui/tiptap/DeferredTiptapEditor` so the Tiptap/ProseMirror bundle loads only after the user chooses to edit. Do not import `TiptapEditor` directly into route-reachable form chunks unless the immediate editor load is an intentional, measured tradeoff.
