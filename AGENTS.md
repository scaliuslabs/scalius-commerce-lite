...
- Order list first load must not eagerly import interaction-only heavy UI. Keep the date-range picker/calendar and bulk-shipping dialog behind user-triggered lazy boundaries; verify plain `/admin/orders` load has no initial `DateRangePickerWithPresets`, `react-day-picker`, or `BulkShipDialog` module request before moving similar code back into the route graph.
