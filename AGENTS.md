...
- **2FA schema and method changes**: Better Auth's `twoFactor` table requires the `verified` column in `packages/database/src/schema/auth.ts` and migrations. Changing the preferred admin 2FA method through `/api/v1/admin/auth/2fa/method` must verify a code for the target method in the API route before updating `user.twoFactorMethod`; do not trust a browser-only prior verification step.
