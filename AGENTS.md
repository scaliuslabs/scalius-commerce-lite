...
- Category pages must build product-list options before the first await, then start layout, category, product-list, filter-metadata, and category-widget reads in one promise wave. Category widgets may chain from the category promise because they require the category id, but product and filter reads must not wait for the standalone category lookup. Keep `apps/storefront/src/lib/page-data-boundaries.test.ts` aligned with this cold-cache latency contract until a consolidated category render-data endpoint replaces the separate calls.
