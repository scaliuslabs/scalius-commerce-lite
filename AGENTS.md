...
- **Payment session proof**: Stripe, SSLCommerz, and Polar session creation must require the order receipt token, validate it against `order_receipt:{token}` before gateway settings/provider calls, and derive callback/success/cancel URLs from trusted runtime config such as `PUBLIC_API_BASE_URL`, not request-body URLs.
