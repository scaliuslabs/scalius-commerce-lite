// Migration 0097 resets stored theme documents to version 5: a published
...
const MIGRATION = "0097_theme_document_v5";
...
describe(MIGRATION, () => {
    it.each(["d1", "turso"] as const)("drops a version 4 store's theme rows on %s", (provider) => {
        const sqlite = createMigratedSqlite({ provider, beforeMigration: "0097_" });
        sqlite.exec(seed(V4));
        sqlite.exec(compiledMigrationSql(provider, undefined, "0097_"));
        ...
        expect(sqlite.prepare("SELECT version, name FROM scalius_schema_migrations ORDER BY version DESC LIMIT 1").get())
            .toEqual({ version: 97, name: MIGRATION });
    });
