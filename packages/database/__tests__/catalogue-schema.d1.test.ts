// Catalogue schema guards (0090) exercised with raw SQL against the real
...
describe.each(["d1", "turso"] as const)("category tree (0090, %s)", (provider) => {
...
describe.each(["d1", "turso"] as const)("catalogue schema upgrade (0090, %s)", (provider) => {
  it("backfills the tree and the content blocks from a pre-0090 store", () => {
    const sqlite = createMigratedSqlite({ provider, beforeMigration: "0090_" });
    sqlite.exec(`
    ...
    `);
    sqlite.exec(compiledMigrationSql(provider, undefined, "0090_"));
...
describe("typed attributes, projections and bundles (0090)", () => {
  function store() {
