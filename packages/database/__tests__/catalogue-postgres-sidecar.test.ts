// PostgreSQL sidecar parity for 0090_catalogue_schema: an 0089 schema upgraded
...
const sidecar = join(import.meta.dirname, "../migrations/postgres/0090_catalogue_schema.sql");
...
async function applySidecar(client: Client): Promise<void> {
  ...
  try {
    ...
    throw new Error(`0090_catalogue_schema: ${(error as Error).message}`);
  }
}
...
describe.runIf(postgresUrl)("0090 catalogue schema PostgreSQL sidecar", () => {
  ...
  it("backfills and maintains the tree, the typed values and the content mirror", async () => {
    const before = await compileCanonicalPostgresSchema({ beforeMigration: "0090_" });
