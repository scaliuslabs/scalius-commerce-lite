...
ALTER TABLE `categories` ADD `parent_id` text REFERENCES categories(id) ON DELETE restrict;
--> statement-breakpoint
ALTER TABLE `categories` ADD `depth` integer DEFAULT 0 NOT NULL CONSTRAINT `categories_depth_range` CHECK (`depth` BETWEEN 0 AND 3);
--> statement-breakpoint
ALTER TABLE `categories` ADD `path` text DEFAULT '' NOT NULL;
--> statement-breakpoint
ALTER TABLE `categories` ADD `listing_template` text CONSTRAINT `categories_listing_template_shape` CHECK (`listing_template` IS NULL OR (length(`listing_template`) BETWEEN 1 AND 40 AND `listing_template` = lower(`listing_template`) AND `listing_template` NOT GLOB '*[^A-Za-z0-9_-]*' AND instr(`listing_template`, '_') = 0));
--> statement-breakpoint
...
--> statement-breakpoint
-- Search only indexes name and description; tree maintenance rewrites depth
-- and path, which must not churn (or, inside the insert, race) the index.
DROP TRIGGER `categories_fts_before_update`;
--> statement-breakpoint
DROP TRIGGER `categories_fts_after_update`;
--> statement-breakpoint
CREATE TRIGGER `categories_fts_before_update` BEFORE UPDATE OF `name`, `description` ON `categories` BEGIN
  INSERT INTO categories_fts(categories_fts, rowid, name, description) VALUES('delete', old.rowid, old.name, old.description);
END;
--> statement-breakpoint
CREATE TRIGGER `categories_fts_after_update` AFTER UPDATE OF `name`, `description` ON `categories` BEGIN
...
BEGIN
  SELECT RAISE(ABORT, 'category id is immutable');
END;
--> statement-breakpoint
...
BEGIN
  SELECT RAISE(ABORT, 'category parent must be another live category');
END;
--> statement-breakpoint
...
BEGIN
  SELECT RAISE(ABORT, 'category tree is limited to four levels');
END;
--> statement-breakpoint
...
BEGIN
  SELECT RAISE(ABORT, 'category cannot move under itself or its descendants');
END;
--> statement-breakpoint
...
WHEN NEW.`path` NOT LIKE '%/' || NEW.`id` || '/'
  OR substr(NEW.`path`, 1, 1) <> '/'
  OR NEW.`depth` <> length(NEW.`path`) - length(replace(NEW.`path`, '/', '')) - 2
BEGIN
  SELECT RAISE(ABORT, 'category depth and path are maintained by the tree triggers');
END;
--> statement-breakpoint
...
CREATE TABLE `brands` (
	...
	`exclude_from_sitemap` integer DEFAULT 0 NOT NULL,
	...
	CONSTRAINT "brands_id_shape" CHECK(substr("brands"."id", 1, 4) = 'brd_' AND length("brands"."id") BETWEEN 10 AND 68 AND "brands"."id" NOT GLOB '*[^A-Za-z0-9_-]*'),
	CONSTRAINT "brands_name_valid" CHECK("brands"."name" = trim("brands"."name") AND length("brands"."name") BETWEEN 1 AND 120),
	CONSTRAINT "brands_slug_valid" CHECK(length("brands"."slug") BETWEEN 1 AND 100 AND "brands"."slug" = lower("brands"."slug") AND "brands"."slug" NOT GLOB '*[^A-Za-z0-9_-]*' AND instr("brands"."slug", '_') = 0),
	...
	CONSTRAINT "brands_listing_template_shape" CHECK("brands"."listing_template" IS NULL OR (length("brands"."listing_template") BETWEEN 1 AND 40 AND "brands"."listing_template" = lower("brands"."listing_template") AND "brands"."listing_template" NOT GLOB '*[^A-Za-z0-9_-]*' AND instr("brands"."listing_template", '_') = 0)),
...
--> statement-breakpoint
ALTER TABLE `products` ADD `brand_id` text REFERENCES brands(id) ON DELETE set null;
--> statement-breakpoint
...
ALTER TABLE `products` ADD `page_template` text CONSTRAINT `products_page_template_shape` CHECK (`page_template` IS NULL OR (length(`page_template`) BETWEEN 1 AND 40 AND `page_template` = lower(`page_template`) AND `page_template` NOT GLOB '*[^A-Za-z0-9_-]*' AND instr(`page_template`, '_') = 0));
--> statement-breakpoint
ALTER TABLE `products` ADD `emi_eligible` integer DEFAULT 1 NOT NULL CONSTRAINT `products_emi_eligible_check` CHECK (`emi_eligible` IN (0, 1));
--> statement-breakpoint
ALTER TABLE `collections` ADD `listing_template` text CONSTRAINT `collections_listing_template_shape` CHECK (`listing_template` IS NULL OR (length(`listing_template`) BETWEEN 1 AND 40 AND `listing_template` = lower(`listing_template`) AND `listing_template` NOT GLOB '*[^A-Za-z0-9_-]*' AND instr(`listing_template`, '_') = 0));
--> statement-breakpoint
...
CREATE TABLE `attribute_groups` (
	...
	`updated_at` integer DEFAULT (cast(strftime('%s','now') as int)) NOT NULL,
	`deleted_at` integer,
	CONSTRAINT "attribute_groups_id_shape" CHECK(substr("attribute_groups"."id", 1, 4) = 'atg_' AND length("attribute_groups"."id") BETWEEN 10 AND 68 AND "attribute_groups"."id" NOT GLOB '*[^A-Za-z0-9_-]*'),
...
--> statement-breakpoint
ALTER TABLE `product_attributes` ADD `group_id` text REFERENCES attribute_groups(id) ON DELETE set null;
--> statement-breakpoint
ALTER TABLE `product_attributes` ADD `value_type` text DEFAULT 'text' NOT NULL CONSTRAINT `product_attributes_value_type_check` CHECK (`value_type` IN ('text', 'number', 'boolean', 'enum'));
--> statement-breakpoint
ALTER TABLE `product_attributes` ADD `unit` text CONSTRAINT `product_attributes_unit_check` CHECK (`unit` IS NULL OR (`unit` = trim(`unit`) AND length(`unit`) BETWEEN 1 AND 16));
--> statement-breakpoint
ALTER TABLE `product_attributes` ADD `sort_order` integer DEFAULT 0 NOT NULL;
--> statement-breakpoint
ALTER TABLE `product_attributes` ADD `key_spec` integer DEFAULT 0 NOT NULL CONSTRAINT `product_attributes_key_spec_check` CHECK (`key_spec` IN (0, 1));
--> statement-breakpoint
ALTER TABLE `product_attributes` ADD `highlight` integer DEFAULT 0 NOT NULL CONSTRAINT `product_attributes_highlight_check` CHECK (`highlight` IN (0, 1));
--> statement-breakpoint
ALTER TABLE `product_attributes` ADD `facet_display` text DEFAULT 'checkbox' NOT NULL CONSTRAINT `product_attributes_facet_display_check` CHECK (`facet_display` IN ('checkbox', 'range', 'swatch', 'search_list') AND (`facet_display` <> 'range' OR `value_type` = 'number') AND (`facet_display` <> 'swatch' OR `value_type` = 'enum'));
--> statement-breakpoint
...
CREATE TABLE `attribute_values` (
	...
	CONSTRAINT "attribute_values_id_shape" CHECK(substr("attribute_values"."id", 1, 4) = 'atv_' AND length("attribute_values"."id") BETWEEN 10 AND 68 AND "attribute_values"."id" NOT GLOB '*[^A-Za-z0-9_-]*'),
	...
	CONSTRAINT "attribute_values_swatch_hex_valid" CHECK("attribute_values"."swatch_hex" IS NULL OR (length("attribute_values"."swatch_hex") = 7 AND "attribute_values"."swatch_hex" GLOB '#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]'))
);
--> statement-breakpoint
...
ALTER TABLE `product_attribute_values` ADD `value_id` text REFERENCES attribute_values(id) ON DELETE restrict;
--> statement-breakpoint
ALTER TABLE `product_attribute_values` ADD `value_number` real;
--> statement-breakpoint
...
WHEN (NEW.`value_id` IS NOT NULL AND NOT EXISTS (
...
BEGIN
  SELECT RAISE(ABORT, 'attribute value does not match the attribute type');
END;
--> statement-breakpoint
...
CREATE TABLE `product_buyer_state` (
	...
	`to_minor` integer,
	`base_minor` integer,
	`discount_depth_bps` integer DEFAULT 0 NOT NULL,
	`has_discount` integer DEFAULT 0 NOT NULL,
	`available_for_sale` integer DEFAULT 0 NOT NULL,
	`has_customer_options` integer DEFAULT 0 NOT NULL,
	`availability_band` text DEFAULT 'out_of_stock' NOT NULL,
	`refreshed_at` integer DEFAULT (cast(strftime('%s','now') as int)) NOT NULL,
	FOREIGN KEY (`product_id`) REFERENCES `products`(`id`) ON UPDATE no action ON DELETE cascade,
...
CREATE TABLE `product_recommendations` (
	...
	`computed_at` integer DEFAULT (cast(strftime('%s','now') as int)) NOT NULL,
...
CREATE TABLE `product_content_blocks` (
	...
	`updated_at` integer DEFAULT (cast(strftime('%s','now') as int)) NOT NULL,
	FOREIGN KEY (`product_id`) REFERENCES `products`(`id`) ON UPDATE no action ON DELETE cascade,
	CONSTRAINT "product_content_blocks_id_shape" CHECK(substr("product_content_blocks"."id", 1, 4) = 'pcb_' AND length("product_content_blocks"."id") BETWEEN 8 AND 120 AND "product_content_blocks"."id" NOT GLOB '*[^A-Za-z0-9_-]*'),
	...
	CONSTRAINT "product_content_blocks_type_shape" CHECK(length("product_content_blocks"."type") BETWEEN 1 AND 40 AND "product_content_blocks"."type" = lower("product_content_blocks"."type") AND "product_content_blocks"."type" NOT GLOB '*[^A-Za-z0-9_-]*' AND instr("product_content_blocks"."type", '_') = 0),
...
SELECT 'pcb_' || `id`, `product_id`, 'tabs', max(0, CAST(`sort_order` AS INTEGER)), 'rich-text', 1,
...
BEGIN
  INSERT INTO `product_content_blocks` (`id`, `product_id`, `placement`, `position`, `type`, `version`, `settings`, `created_at`, `updated_at`)
  VALUES ('pcb_' || NEW.`id`, NEW.`product_id`, 'tabs', max(0, CAST(NEW.`sort_order` AS INTEGER)), 'rich-text', 1,
...
BEGIN
  ...
  SET `position` = max(0, CAST(NEW.`sort_order` AS INTEGER)),
...
CREATE TABLE `product_bundles` (
	...
	`updated_at` integer DEFAULT (cast(strftime('%s','now') as int)) NOT NULL,
	FOREIGN KEY (`product_id`) REFERENCES `products`(`id`) ON UPDATE no action ON DELETE cascade,
	CONSTRAINT "product_bundles_id_shape" CHECK(substr("product_bundles"."id", 1, 4) = 'pbd_' AND length("product_bundles"."id") BETWEEN 10 AND 68 AND "product_bundles"."id" NOT GLOB '*[^A-Za-z0-9_-]*'),
...
--> statement-breakpoint
INSERT INTO `scalius_schema_migrations` (`version`, `name`, `source_sha256`) VALUES (90, '0090_catalogue_schema', '0c4e0fe4d503aaeda11d492acb521f8740e64073427df0c2d2e2ed9802bf0ecf');
