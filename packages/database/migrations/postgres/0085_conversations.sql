...
CREATE TABLE "conversations" (
	...
	"customer_read_seq" bigint DEFAULT 0 NOT NULL,
	"staff_read_seq" bigint DEFAULT 0 NOT NULL,
	...
	FOREIGN KEY ("assignee_user_id") REFERENCES "user"("id") ON UPDATE no action ON DELETE set null DEFERRABLE INITIALLY DEFERRED,
...
CREATE OR REPLACE FUNCTION scalius_compat."conversation_messages_seq_insert_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  ...
  WHERE c."id" = NEW."conversation_id" AND c."last_seq" = NEW."seq"
    ...
    RETURN NEW;
  END IF;
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'message seq must equal the conversation sequence';
  RETURN NEW;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."conversation_messages_seq_insert_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."conversation_messages_update_blocked_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'conversation messages are append-only';
  RETURN NEW;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."conversation_messages_update_blocked_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."conversation_messages_delete_blocked_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."conversation_messages_delete_blocked_fn"();
--> statement-breakpoint
...
CREATE OR REPLACE FUNCTION scalius_compat."conversations_seq_advance_guard_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((NEW."last_seq" <> OLD."last_seq"
  AND (
    ...
    RETURN NEW;
  END IF;
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'conversation sequence advances one message at a time';
  RETURN NEW;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."conversations_seq_advance_guard_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."conversations_identity_immutable_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((NEW."subject_type" IS DISTINCT FROM OLD."subject_type"
  OR NEW."subject_id" IS DISTINCT FROM OLD."subject_id"
  OR NEW."order_id" IS DISTINCT FROM OLD."order_id"), false) THEN
    RETURN NEW;
  END IF;
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'conversation subject is immutable';
  RETURN NEW;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."conversations_identity_immutable_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."conversations_delete_blocked_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'conversations are durable records';
  RETURN OLD;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."conversations_delete_blocked_fn"();
--> statement-breakpoint
CREATE TABLE "conversation_attachments" (
	...
	"attached_at" bigint,
	FOREIGN KEY ("conversation_id") REFERENCES "conversations"("id") ON UPDATE no action ON DELETE restrict DEFERRABLE INITIALLY DEFERRED,
	FOREIGN KEY ("message_id") REFERENCES "conversation_messages"("id") ON UPDATE no action ON DELETE restrict DEFERRABLE INITIALLY DEFERRED,
...
CREATE OR REPLACE FUNCTION scalius_compat."conversation_attachments_attach_guard_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((OLD."message_id" IS NOT NULL
  OR NEW."message_id" IS NULL
  OR NEW."id" IS DISTINCT FROM OLD."id"
  OR NEW."conversation_id" IS DISTINCT FROM OLD."conversation_id"
  OR NEW."uploader_type" IS DISTINCT FROM OLD."uploader_type"
  OR NEW."uploader_ref" IS DISTINCT FROM OLD."uploader_ref"
  ...
  OR (
    ...
    RETURN NEW;
  END IF;
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'attachment attaches once to a message of its conversation, at most three per message';
  RETURN NEW;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."conversation_attachments_attach_guard_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."conversation_attachments_delete_guard_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((OLD."message_id" IS NOT NULL), false) THEN
    RETURN OLD;
  END IF;
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'attached attachments are part of the conversation record';
  RETURN OLD;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."conversation_attachments_delete_guard_fn"();
--> statement-breakpoint
ALTER TABLE "order_support_requests" ADD COLUMN "conversation_id" text REFERENCES "conversations"("id") ON UPDATE no action ON DELETE restrict DEFERRABLE INITIALLY DEFERRED;
--> statement-breakpoint
