...
ORDER BY
  redemption."discount_id",
  order_record."account_owner_customer_id",
  redemption."created_at" ASC,
  redemption."order_id" ASC
ON CONFLICT ("discount_id", "customer_key") DO NOTHING;
--> statement-breakpoint
DROP TRIGGER IF EXISTS "discount_usage_one_per_customer_guard" ON "discount_usage";
--> statement-breakpoint
DROP FUNCTION IF EXISTS scalius_compat."discount_usage_one_per_customer_guard_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."discount_usage_customer_identity_guard_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((
    SELECT "limit_one_per_customer" = 1
    FROM "discounts"
  ...
  IF NOT EXISTS (
    ...
    WHERE new_order."id" = NEW."order_id"
      AND NULLIF(BTRIM(new_order."customer_phone"), '') IS NOT NULL
  ) THEN
    RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'DISCOUNT_CUSTOMER_KEY_REQUIRED';
  END IF;
  RETURN NEW;
END
$trigger_function$;
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."discount_usage_customer_identity_guard_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."discount_usage_one_per_customer_guard_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE redemption."discount_id" = NEW."discount_id"
      AND (
        redemption."customer_key" = 'phone:' || BTRIM(new_order."customer_phone")
        OR (
    ...
    RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'DISCOUNT_ONE_PER_CUSTOMER_EXCEEDED';
  END IF;
  RETURN NEW;
END
$trigger_function$;
...
BEFORE INSERT ON "discount_usage"
FOR EACH ROW EXECUTE FUNCTION scalius_compat."discount_usage_one_per_customer_guard_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."discount_usage_customer_redemption_claim_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  ...
  SELECT
    NEW."discount_id",
    'phone:' || BTRIM(new_order."customer_phone"),
    NEW."order_id",
    NEW."customer_id",
    COALESCE(NEW."created_at", extract(epoch from now())::bigint)
  FROM "orders" AS new_order
  WHERE new_order."id" = NEW."order_id"
  UNION ALL
  SELECT
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."discount_usage_customer_redemption_claim_fn"();
--> statement-breakpoint
