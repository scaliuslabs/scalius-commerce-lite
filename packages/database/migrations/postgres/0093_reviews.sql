...
CREATE TABLE "product_reviews" (
	...
	"reply_user_id" text,
	"replied_at" bigint,
	"edit_count_day" bigint DEFAULT 0 NOT NULL,
	"edit_day" bigint,
	"edited_at" bigint,
	...
	FOREIGN KEY ("variant_id") REFERENCES "product_variants"("id") ON UPDATE no action ON DELETE set null DEFERRABLE INITIALLY DEFERRED,
	FOREIGN KEY ("order_id") REFERENCES "orders"("id") ON UPDATE no action ON DELETE cascade DEFERRABLE INITIALLY DEFERRED,
	FOREIGN KEY ("order_item_id") REFERENCES "order_items"("id") ON UPDATE no action ON DELETE cascade DEFERRABLE INITIALLY DEFERRED,
	FOREIGN KEY ("customer_id") REFERENCES "customers"("id") ON UPDATE no action ON DELETE set null DEFERRABLE INITIALLY DEFERRED,
	FOREIGN KEY ("reply_user_id") REFERENCES "user"("id") ON UPDATE no action ON DELETE set null DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE "product_review_stats" (
	...
	"count_1" bigint DEFAULT 0 NOT NULL,
	"count_2" bigint DEFAULT 0 NOT NULL,
	"count_3" bigint DEFAULT 0 NOT NULL,
	"count_4" bigint DEFAULT 0 NOT NULL,
	"count_5" bigint DEFAULT 0 NOT NULL,
	"rating_avg_centi" bigint,
	"rating_rank_milli" bigint,
...
CREATE OR REPLACE FUNCTION scalius_compat."product_reviews_line_eligible_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  ...
  WHERE i."id" = NEW."order_item_id"
    ...
    RETURN NEW;
  END IF;
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'review requires a fulfilled line of a delivered order';
  RETURN NEW;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."product_reviews_line_eligible_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."product_reviews_stats_row_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((NOT EXISTS (SELECT 1 FROM "product_review_stats" AS s WHERE s."product_id" = NEW."product_id")), false) THEN
    RETURN NEW;
  END IF;
...
BEFORE INSERT ON "product_reviews"
FOR EACH ROW EXECUTE FUNCTION scalius_compat."product_reviews_stats_row_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."product_reviews_stats_add_insert_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((NEW."status" = 'published'), false) THEN
    RETURN NEW;
  END IF;
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."product_reviews_stats_add_insert_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."product_reviews_stats_sub_update_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((OLD."status" = 'published'), false) THEN
    RETURN NEW;
  END IF;
  UPDATE "product_review_stats"
  SET "review_count" = "review_count" - 1,
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."product_reviews_stats_sub_update_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."product_reviews_stats_add_update_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
...
AFTER UPDATE OF "status", "rating" ON "product_reviews"
FOR EACH ROW EXECUTE FUNCTION scalius_compat."product_reviews_stats_add_update_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."product_reviews_stats_sub_delete_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."product_reviews_stats_sub_delete_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."product_reviews_identity_immutable_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  ...
  OR NEW."reviewer_key" IS DISTINCT FROM OLD."reviewer_key"), false) THEN
    RETURN NEW;
  END IF;
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'review identity is immutable';
  RETURN NEW;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."product_reviews_identity_immutable_fn"();
--> statement-breakpoint
...
CREATE OR REPLACE FUNCTION scalius_compat."orders_delivered_review_request_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((NEW."status" = 'delivered'
  AND OLD."status" IS DISTINCT FROM 'delivered'
  AND NOT EXISTS (SELECT 1 FROM "order_review_requests" AS r WHERE r."order_id" = NEW."id")), false) THEN
    RETURN NEW;
  END IF;
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."orders_delivered_review_request_fn"();
--> statement-breakpoint
INSERT INTO "scalius_schema_migrations" ("version", "name", "source_sha256") VALUES (93, '0093_reviews', 'f0fa69308e86a712f5bcb7c80e126f17d15dbe449fcd3e5f6996100f9dd644f0');
