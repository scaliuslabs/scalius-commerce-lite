CREATE TABLE "agent_browser_handoffs" (
	...
	FOREIGN KEY ("owner_user_id") REFERENCES "user"("id") ON UPDATE no action ON DELETE no action DEFERRABLE INITIALLY DEFERRED,
...
CREATE INDEX "agent_browser_handoffs_owner_status_expiry_idx" ON "agent_browser_handoffs" ("owner_user_id","status","expires_at");
--> statement-breakpoint
CREATE INDEX "agent_browser_handoffs_grant_status_expiry_idx" ON "agent_browser_handoffs" ("grant_id","status","expires_at");
--> statement-breakpoint
CREATE INDEX "agent_browser_handoffs_status_expiry_idx" ON "agent_browser_handoffs" ("status","expires_at");
--> statement-breakpoint
