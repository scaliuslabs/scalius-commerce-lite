...
CREATE TABLE "digital_assets" (
	...
	"size_bytes" bigint,
	"current_r2_key" text,
	"download_limit" bigint DEFAULT 5,
	"access_days" bigint,
...
CREATE TABLE "digital_asset_uploads" (
	...
	"created_at" bigint DEFAULT (cast(strftime('%s','now') as bigint)) NOT NULL,
	"updated_at" bigint DEFAULT (cast(strftime('%s','now') as bigint)) NOT NULL,
	FOREIGN KEY ("asset_id") REFERENCES "digital_assets"("id") ON UPDATE no action ON DELETE cascade DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE "digital_entitlements" (
	...
	FOREIGN KEY ("asset_id") REFERENCES "digital_assets"("id") ON UPDATE no action ON DELETE restrict DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE "digital_licence_keys" (
	...
	FOREIGN KEY ("entitlement_id") REFERENCES "digital_entitlements"("id") ON UPDATE no action ON DELETE restrict DEFERRABLE INITIALLY DEFERRED,
...
CREATE OR REPLACE FUNCTION scalius_compat."digital_licence_keys_transition_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  IF NOT coalesce((NOT (OLD."status" = 'available' AND NEW."status" IN ('assigned', 'revoked'))
  OR NEW."id" IS DISTINCT FROM OLD."id"
  OR NEW."asset_id" IS DISTINCT FROM OLD."asset_id"
  OR NEW."key_ciphertext" IS DISTINCT FROM OLD."key_ciphertext"
  OR NEW."key_hash" IS DISTINCT FROM OLD."key_hash"
  OR NEW."key_last4" IS DISTINCT FROM OLD."key_last4"
  OR NEW."import_id" IS DISTINCT FROM OLD."import_id"), false) THEN
    RETURN NEW;
  END IF;
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'licence keys only move from available to assigned or revoked';
  RETURN NEW;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."digital_licence_keys_transition_fn"();
--> statement-breakpoint
CREATE OR REPLACE FUNCTION scalius_compat."digital_licence_keys_delete_blocked_fn"()
RETURNS trigger
LANGUAGE plpgsql
AS $trigger_function$
BEGIN
  RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'licence keys are durable records, revoke unused keys instead';
  RETURN OLD;
END
$trigger_function$;
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION scalius_compat."digital_licence_keys_delete_blocked_fn"();
--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO "scalius_schema_migrations" ("version", "name", "source_sha256") VALUES (94, '0094_digital_goods', 'b78be4c196d831cc0fc824fd82c1e66dbc4a9a845b3bef256bde87ee0c5ab718');
