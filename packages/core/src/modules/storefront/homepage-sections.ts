// Homepage section reads: the product lists (newest, on sale, popular, a
// category) and the images (banners, lookbook and editorial photos, hero
// side banners) the theme's sections show. Everything here is planned as
// statements for the homepage's second D1 batch, beside the collection
// reads: each product statement is scoped to the products it returns (the
// buyer pricing projection never ranks the whole catalogue) and carries a
// media statement for exactly the same rows, so cards need no third wave.
// Lists come from `homeSectionRequests` (@scalius/shared/storefront-theme).
import { categories, media, orderItems, orders, products } from "@scalius/database/schema";
...
import { buildBuyerCatalogPricingProjection } from "../products/products.buyer-projection";
import {
