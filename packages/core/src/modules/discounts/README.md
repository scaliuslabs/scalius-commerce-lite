...
8. **No percentage cap**: A percentage discount of 200% would result in the `Math.min(subtotal, calculated)` cap preventing negative totals, but no validation prevents values > 100 for percentage type.
