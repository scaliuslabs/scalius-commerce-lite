...
describe("discount usage D1 authority", () => {
  ...
  function createDatabase(): DatabaseSync {
    ...
    `);
    sqlite.exec(triggerSql("discount_usage_max_uses_guard"));
    sqlite.exec(triggerSql("discount_usage_one_per_customer_guard"));
    return sqlite;
  }
  ...
  it("atomically rejects a redemption after the total usage limit", () => {
    ...
    db.prepare("INSERT INTO orders VALUES ('order_1', '+8801712345678', 'cust_1')").run();
    db.prepare("INSERT INTO orders VALUES ('order_2', '+8801812345678', 'cust_2')").run();
  ...
  it("uses an immutable trimmed phone claim for one-per-customer redemption", () => {
    ...
    db.prepare("INSERT INTO orders VALUES ('order_1', ' +8801712345678 ', 'cust_1')").run();
    db.prepare("INSERT INTO orders VALUES ('order_2', '+8801712345678', 'cust_2')").run();
    db.prepare("INSERT INTO orders VALUES ('order_3', '+8801712345678', 'cust_3')").run();
  ...
  it("rejects a one-per-customer redemption without a phone identity", () => {
    const db = createDatabase();
    db.prepare("INSERT INTO discounts VALUES ('disc_phone', NULL, 1)").run();
    db.prepare("INSERT INTO orders VALUES ('order_blank', '   ', 'cust_1')").run();
