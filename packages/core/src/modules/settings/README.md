...
| `getStorefrontUrlSetting` | `(db) => { storefrontUrl }` | Reads the saved storefront value without inventing a relative fallback; missing state is an empty string and invalid legacy state remains visible for correction |
| `saveStorefrontUrl` | `(db, url) => void` | Normalizes and upserts one HTTPS origin (HTTP loopback only for local development); rejects credentials, paths, queries, fragments, blanks, and the legacy `/` sentinel |
