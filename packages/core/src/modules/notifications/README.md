...
Auth OTP shares the same provider-health marker for merchant-actionable setup failures, but auth OTP DLQ archival, receipt-backoff-aware Queue retry delays, and preserving provider-accepted OTP responses across receipt-write failures remain follow-up reliability work.
