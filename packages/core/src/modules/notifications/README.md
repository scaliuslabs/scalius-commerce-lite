...
- Configuration/readiness failures are not retry work. Missing email provider setup, no active SMS provider, thrown SMS/WhatsApp setup/decrypt failures, undecryptable credentials, invalid keys/tokens, authorization failures, sender/account/balance blockers, and bad Firebase credentials are skipped receipts with merchant-actionable admin copy. Transient provider outages remain failed/retryable, but receipt attempts are capped so unknown provider failures settle as `delivery_attempt_limit_reached` instead of retrying forever.
