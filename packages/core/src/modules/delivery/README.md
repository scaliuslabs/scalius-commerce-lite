...
| Function | Signature | Notes |
|----------|-----------|-------|
| `updateOrderStatusFromShipment` | `(db, shipmentId, newStatus)` | Maps shipment status to order status, applies inventory side-effects via `applyInventoryForStatusChange()` |
...
Before updating, calls `applyInventoryForStatusChange()` for inventory side-effects.
