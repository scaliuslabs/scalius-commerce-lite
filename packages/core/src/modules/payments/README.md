...
1. **Session creation level**: Public Stripe, SSLCommerz, and Polar routes claim `payment_session_attempts` before provider calls using a canonical key derived from order id, receipt token hash, gateway, payment type, server-derived amount/currency, and route-owned callback/customer context. Created attempts store the replay payload (`clientSecret`/redirect URL/session id) so identical retries return the original session without touching the provider again. Live in-flight attempts return a retryable `202 processing` response instead of creating a duplicate or surfacing a hard conflict. Failed attempts are reclaimable. Stripe also receives the same durable attempt key as its provider idempotency key.
