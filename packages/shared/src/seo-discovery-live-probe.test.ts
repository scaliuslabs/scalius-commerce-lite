...
describe("runSeoDiscoveryLiveProbe", () => {
  ...
  it("probes every discovery path without forwarding credentials", async () => {
    mocks.getStorefrontUrl.mockResolvedValue({
    ...
    expect(fetchMock.mock.calls.map((call) => String(call[0]))).toEqual([
      ...
      ...SITEMAP_CHILD_PATHS.map((path) => `https://shop.example.com${path}`),
    ]);
  ...
  it("fails a non-empty feed proof when required item fields are missing or relative", async () => {
    ...
    mocks.getDiscoveryPolicy.mockResolvedValue({
      discovery: { feeds: { productCatalogEnabled: true } },
    });
  ...
  it("skips disabled feeds but always probes robots and every sitemap section", async () => {
    mocks.getStorefrontUrl.mockResolvedValue({
      storefrontUrl: "https://shop.example.com/",
    });
    mocks.getDiscoveryPolicy.mockResolvedValue({
      discovery: { feeds: { productCatalogEnabled: false } },
    });
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith("/robots.txt")) {
    ...
    expect(
      result.resources.some(
        (resource) =>
          resource.kind === "sitemapChild" && resource.disabledReason,
      ),
    ).toBe(false);
  });
  ...
  it("skips the UCP profile on absolute HTTP Store URLs without using the feed toggle", async () => {
    ...
    expect(fetchMock.mock.calls.map((call) => String(call[0]))).toEqual([
      ...
      ...SITEMAP_CHILD_PATHS.map((path) => `http://shop.example.com${path}`),
    ]);
    expect(result.ok).toBe(true);
    expect(result.resources).toEqual(
      expect.arrayContaining([
