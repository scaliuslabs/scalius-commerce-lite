...
As of the 2026-07-05 follow-up check, the repo code path is deployed but routed proof is externally blocked. Live version metadata for ops-monitor version `cc15af3e-23b5-4033-b9cb-1fbc3709ce41` shows `ALERT_EMAIL_FROM` and `ALERT_EMAIL_TO` deployed as empty strings, so the Worker cannot construct an alert email config or send a routed notification. The active Wrangler OAuth token can list the `scalius.com` Email Routing zone, but Email Routing/Sending address and settings calls fail with Cloudflare auth error `10000` because the token is missing `email_routing:write` and `email_sending:write`. Destination addresses also require human email verification before they can be used. After `wrangler login` refreshes those scopes and verified aliases exist, configure `ALERT_EMAIL_FROM` / `ALERT_EMAIL_TO`, restrict the `ALERT_EMAIL` binding to those aliases if supported for the account, deploy with `pnpm deploy:ops-monitor`, run a safe Email Service test, and record only aliases/timestamps, never raw personal addresses or inbox screenshots.
