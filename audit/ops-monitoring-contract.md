...
| Queue backlog and DLQs | Cloudflare Queues metrics or GraphQL Analytics for all API queues and DLQs listed below. Include backlog count, oldest message age, lag/retry evidence, and consumer outcome where available. The repo smoke must also prove provider-side producer/consumer wiring with `pnpm ops:check --queues` so missing queue consumers are not hidden behind dashboard-only review. | Page when any `*-dlq` backlog is non-zero for 5 minutes, any normal queue has oldest message age above `300000ms`, backlog grows for 15 minutes, completions repeatedly report retries, or `pnpm ops:check --queues` reports missing expected `worker:scalius-api` wiring. |
...
| Queue monitoring covers every queue | `pnpm ops:check --queues --samples 1 --timeout-ms 20000` output summary showing `monitoringConfig` passed and every queue reports expected `worker:scalius-api` producer/consumer wiring, plus provider-side backlog/oldest-age evidence for each normal queue and DLQ. |
