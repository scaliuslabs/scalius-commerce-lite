...
Then capture Wrangler tail or Workers Logs evidence for the scheduled monitor run: `/readyz` result, queue/DLQ `Queue.metrics()` summary, KV streak/cooldown decisions, `alertCount`, `routedAlertCount`, `deliveryFailureCount`, and redacted structured log fields. If Cloudflare Email Service routing is enabled, record the routed test alert timestamp and destination alias without tokens, inbox screenshots, or raw personal email addresses. If delivery fails, use `ops_monitor.alert_delivery_failed` with its monitor id/run id/error code; do not retry by lowering thresholds until sender/destination verification is understood.
