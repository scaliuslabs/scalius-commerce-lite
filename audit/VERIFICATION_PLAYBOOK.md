...
`pnpm ops:check`, `pnpm release:check`, and API deploy verification all use the same four-sample `/readyz` recovery window by default: the final sample must be ready and at least two samples must be ready. `pnpm release:check` also validates every emitted feed item's link, image, availability, direct positive item price, and positive same-currency lower sale price while ignoring nested shipping prices, validates emitted homepage OnlineStore/WebSite/SearchAction/MerchantReturnPolicy JSON-LD, verifies the deployed agent Worker `/health` plus MCP tools/list as catalog-only unless `--skip-live` or `--skip-wrangler` was explicitly used, proves Admin MCP fails closed without a dashboard session, optionally signs in and smoke-tests authenticated Admin MCP when `SCALIUS_RELEASE_ADMIN_EMAIL` and `SCALIUS_RELEASE_ADMIN_PASSWORD` are present, then validates the storefront UCP profile as HTTPS/catalog-only and runs read-only UCP catalog search/lookup/product checks when discovery exposes a product candidate.
...
Required evidence before closing OPS-005/OPS-008 monitoring scope: deployed ops-monitor Worker version, ops-monitor cron schedule (`*/2 * * * *`), no-public-route confirmation, KV binding for streak/cooldown/proof-result state, Cloudflare Email Service `ALERT_EMAIL` binding, every normal/DLQ queue binding, Wrangler tail or Workers Logs showing scheduled `/readyz` checks and `Queue.metrics()` summaries, redacted structured log examples, and routed Cloudflare Email Service proof from the scheduled KV proof marker. Keep this distinct from the API scheduled-maintenance cron (`*/15 * * * *`). The monitor logs `alertCount`, `routedAlertCount`, and `deliveryFailureCount`; do not treat logs-only alert events as routed notification success. Logs-only monitoring is acceptable as an intermediate slice, but the tracker must keep alert-channel verification open until a verified sender/destination test notification is recorded without secrets or inbox screenshots.

Stable-release owner actions before clearing OPS-005 and OPS-010:

- OPS-005: Refresh Cloudflare Email Service scopes with `pnpm --dir apps/ops-monitor exec wrangler login`, then use read-only checks such as `pnpm --dir apps/ops-monitor exec wrangler email routing list`, `pnpm --dir apps/ops-monitor exec wrangler email routing settings scalius.com`, `pnpm --dir apps/ops-monitor exec wrangler email routing addresses list`, `pnpm --dir apps/ops-monitor exec wrangler email sending list scalius.com`, and `pnpm --dir apps/ops-monitor exec wrangler email sending settings scalius.com` to prove the account can manage Email Routing/Sending. Create and verify the ops sender and destination in Cloudflare, configure `ALERT_EMAIL_FROM` / `ALERT_EMAIL_TO` from verified aliases, restrict the `ALERT_EMAIL` binding to those verified sender/destination addresses when the account supports it, and deploy with `pnpm deploy:ops-monitor`. To prove routing without waiting for a real outage, set the one-shot `OPS_MONITOR_STATE` KV marker below, wait for the next scheduled run, and capture the redacted result key plus a scheduled-run log with `routedAlertCount:1` and `deliveryFailureCount:0`. Do not use a temporary bad `READYZ_URL` deploy for this proof, and do not put real addresses, inbox screenshots, provider payloads, or secrets in the tracker.
...
The first public agent Worker is intentionally stateless and read-only. It must expose only `GET /health` and `/mcp` read tools backed by storefront UCP catalog/profile endpoints, compact public category reads, public SEO discovery-policy reads, plus the bounded `cart_validate` snapshot-validation tool, and it must not gain D1, R2, KV, queue, Durable Object, provider-secret, checkout, cart mutation, order, payment, fulfillment, customer, support, or recovery bindings without a new architecture note and focused tests. Admin MCP is not a public agent-host route: `/mcp/admin` is accepted only on the internal service-binding host `http://agent.internal/mcp/admin`, and direct public agent-host requests must return a bland no-store 404 before cookie/API preflight. The dashboard `/api/assistant/mcp` proxy is the only external Admin MCP entry point; it strips bearer auth, forwards only dashboard cookies and MCP protocol headers, and keeps responses no-store. `pnpm run deploy:agent` now performs the live `/health` plus MCP JSON-RPC initialize/tools-list smoke after deploy, and `pnpm release:check` repeats the same catalog/category/discovery-policy plus read-only cart-validation tool-list and tool-call gate for release candidates unless `--skip-live` or `--skip-wrangler` is intentionally used. Storefront UCP discovery remains catalog-only.
...
For live/admin-release smokes, also prove cookie-bearing admin API failures fail fast: `pnpm release:check` sends an invalid `better-auth.session_token` to `/api/v1/admin/settings/business` through both `api.scalius.com` and the dashboard proxy and expects `401`/`403` without `ADMIN_API_READ_TIMEOUT`/`504`, while a real dashboard session should read `/api/v1/admin/orders`, `/api/v1/admin/orders/payment-recovery`, and `/api/v1/admin/orders/payment-recovery/export` through both surfaces without hitting `ADMIN_API_READ_TIMEOUT`.
