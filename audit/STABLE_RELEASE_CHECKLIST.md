...
- Current branch has passed `pnpm release:check` as the compact read-only production release smoke, including UCP profile/search/lookup coverage. Use `--skip-wrangler` only when Cloudflare auth is unavailable and record that limitation.
