...
- Current branch has passed `pnpm release:check` as the compact read-only production release smoke, including public SEO policy, homepage global JSON-LD proof, feeds/sitemaps, and UCP profile/search/lookup/product coverage. Use `--skip-wrangler` only when Cloudflare auth is unavailable and record that limitation. Do not use `--allow-strict-seo-policy-fallback` for release candidates unless the public SEO policy outage is separately accepted and documented.
