...
- `pnpm --filter @scalius/admin-v2 typecheck`: passed after the shared DataTable sortable lazy-loading slice; admin server functions remain in typed domain slices under `api-functions/`.
...
- `pnpm test`: passed 128 files and 802 tests after the shared DataTable sortable lazy-loading slice and storefront purge-cache port-preservation test.
...
- Full `pnpm deploy`: latest shared DataTable sortable lazy-loading slice redeployed API `cc4da6c9-b1ab-4737-91f0-060716ccecb4`, admin `0dae492a-b87e-4043-9558-a564837ecd7e`, and storefront `7a42c38a-3525-4508-a8a3-4c63ccf1c80e`.
- Live HTTP and browser checks: API setup, demo sign-in through `POST /api/auth/sign-in/email`, authenticated `/admin`, authenticated `/admin/orders`, authenticated `/admin/collections?sort=sortOrder&order=asc`, storefront `/`, and storefront `/search` returned successfully after redeploy with no error-boundary page, stuck loading state, or captured console/page/request errors.
- `/admin/orders` and drag-enabled `/admin/collections` local/live performance checks: plain orders page load made no initial `DateRangePickerWithPresets`, `react-day-picker`, `BulkShipDialog`, `DeleteOrderDialog`, `OrderItemsPopover`, `FraudCheckIndicator`, `SortableDataTableContent`, or `sortable.esm` module request. Local collections loaded `SortableDataTableContent.tsx` plus `@dnd-kit` deps on demand; live collections loaded `SortableDataTableContent-BdEuf433.js` and `sortable.esm-CsHTWzZ-.js` only for the sortable route.
...
Fix direction: split this into small, measured slices. Dashboard activity separation, the matching customer activity index, order-detail payment/COD prefetching, checkout inactive-tab prefetch trimming, shell/settings query-options splitting, list-navigation warmups, mutation freshness, dashboard first-paint motion removal, route-facing mutation barrel splitting, orders interaction lazy-loading, and shared DataTable sortable splitting are now done; remaining slices are broader route/query bundle splitting, repeated guard work, heavy chart/editor/export/widget chunks, and deeper route-specific chunk slimming.

Status: In Progress as of 2026-06-15. The first eight slices split dashboard summary/activity, indexed customer activity, warmed order-detail payment/COD data, trimmed checkout inactive-tab prefetching, moved hot shell query options to narrow modules, made common list client navigation non-blocking, tightened mutation invalidation freshness, removed dashboard first-paint motion runtime, and split route-facing mutation hooks by domain. The ninth through eleventh slices moved the orders date-range picker/calendar, bulk-shipping dialog, row item/fraud popovers, and delete confirmation dialog behind lazy interaction boundaries. The twelfth slice moved `@dnd-kit/*` and sortable row behavior out of the shared `DataTable` default path into lazy `SortableDataTableContent`. Verification includes focused dashboard/order-detail/checkout/auth tests, mutation import-boundary/export-parity scans, affected workspace checks, root `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm build`, env/dist-secret/migration checks, production chunk scans, local Chrome `/admin/orders` and `/admin/collections?sort=sortOrder&order=asc` module-request smokes, full deploy to API `cc4da6c9-b1ab-4737-91f0-060716ccecb4`, admin `0dae492a-b87e-4043-9558-a564837ecd7e`, storefront `7a42c38a-3525-4508-a8a3-4c63ccf1c80e`, and live browser smokes proving orders has zero initial sortable/heavy module requests while drag-enabled collections loads `SortableDataTableContent-BdEuf433.js` and `sortable.esm-CsHTWzZ-.js` on demand.
