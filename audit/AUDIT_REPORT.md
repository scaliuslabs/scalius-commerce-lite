...
Fix direction: split this into small, measured slices. Dashboard activity separation, the matching customer activity index, and order-detail payment/COD prefetching are now done; remaining slices are route/query bundle splitting, repeated guard work, checkout settings prefetch trimming, and orders chunk slimming.

Status: In Progress as of 2026-06-15. First slice split `/api/v1/admin/dashboard/summary` from `/api/v1/admin/dashboard/activity`, changed the admin home loader to await only summary data while prefetching activity in the background, kept the legacy combined endpoint, added `customers_dashboard_activity_idx` in migration `0041`, regenerated the production-aligned SDK to 254 paths / 352 operations, and added route tests proving summary does not execute the activity query. Second slice moved admin order-detail payment history, currency, and COD tracking warming into `prefetchOrderDetailQueries()` while keeping optional payment/COD/currency prefetch failures non-fatal. Verification: focused dashboard API tests, focused order-detail prefetch tests, migration metadata check, affected workspace typechecks/lints, root `pnpm test`, root `pnpm typecheck`, root `pnpm build`, local API/admin/storefront browser smoke, `pnpm dev:doctor --require-running`, full deploy to API `e1b0a003-9c47-4e7c-9afd-47a209ed3095`, admin `bc36bca6-0628-47dc-8cab-24c6c3bdf9f9`, storefront `d3b9d1e9-795c-44be-99f0-5ab1ae5d7e89`, live API/product HTTP checks, live authenticated `/admin`, `/admin/orders`, `/admin/orders/6RMK4J`, and live storefront home/product browser smoke. Remaining slices are route/query bundle splitting, repeated guard work, checkout settings prefetch trimming, and orders chunk slimming.
