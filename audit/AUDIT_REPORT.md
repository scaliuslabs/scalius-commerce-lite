...
- `pnpm --filter @scalius/admin-v2 typecheck`: passed after the product-variant interaction-tool deferral slice; admin server functions remain in typed domain slices under `api-functions/`.
...
- `pnpm test`: passed 129 files and 808 tests after the product-variant CSV parser regression tests were added.
...
- Full `pnpm deploy`: latest product-variant interaction-tool deferral slice redeployed API `0b6dd797-a59e-4958-8182-21a4fceb6021`, admin `fbf74d2d-46ba-441a-925a-ca3e9feee222`, and storefront `b67e5b1f-dbf2-4691-9dc5-6af74d28e02e`.
- Live HTTP and browser checks: API setup, public products, unauthenticated dashboard redirect, storefront `/`, demo sign-in through `POST /api/auth/sign-in/email` with dashboard `Origin`, authenticated `/admin/products/prod_WH2tJTRAzGB4jMTZB8hoX/edit`, lazy Bulk Variant Generator, and lazy Reorder Variant Values returned successfully with no captured console/page errors. No production product data was mutated.
- `/admin/orders`, drag-enabled `/admin/collections`, `/admin/discounts/:id/edit`, product forms, and product-variant local/live performance checks: plain orders page load made no initial `DateRangePickerWithPresets`, `react-day-picker`, `BulkShipDialog`, `DeleteOrderDialog`, `OrderItemsPopover`, `FraudCheckIndicator`, `SortableDataTableContent`, or `sortable.esm` module request. Local collections loaded `SortableDataTableContent.tsx` plus `@dnd-kit` deps on demand; live collections loaded `SortableDataTableContent-BdEuf433.js` and `sortable.esm-CsHTWzZ-.js` only for the sortable route. Local discount edit checks covered all three discount types and loaded only the matching form chunk; live discount edit checked the existing `amount_off_order` discount and loaded only `AmountOffOrderForm`. Product edit chunks now keep `bulk-generator`, `VariantSortModal`, and `csvHelpers` as lazy/on-demand code paths.
