...
- `/admin/orders` local and live performance checks: plain page load made no initial `DateRangePickerWithPresets`, `react-day-picker`, or `BulkShipDialog` module request; first date-picker click loaded the picker/calendar and showed `Quick select`; selected-order bulk ship loaded `BulkShipDialog` on demand with no console/page errors.
