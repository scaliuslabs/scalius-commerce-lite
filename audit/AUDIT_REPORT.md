...
- `pnpm test`: passed 128 files and 803 tests after the discount edit lazy-loading slice and catalog purge false-500 regression test.
...
- Full `pnpm deploy`: latest discount-edit/cache slice redeployed API `f9597a03-f87b-4296-9966-cbfecb999c4c`, admin `0e960b5d-cde9-46a8-88ef-ba01fbd1211c`, and storefront `862559f7-d881-4b78-b8a4-6b8a3a68d624`. A follow-up `pnpm run deploy:admin` for the discount-date hydration fix deployed admin `80104b0b-2d5c-47a1-a14f-baca5a6fa7e4`.
- Live HTTP and browser checks: API health, unauthenticated dashboard redirect, storefront `/`, demo sign-in through `POST /api/auth/sign-in/email`, and authenticated `/admin/discounts/disc_IcgNztBsUGqxYDHzqDtm1/edit` for existing `amount_off_order` discount `OFFER250` returned successfully after the final admin deploy with no error-boundary page, stuck loading state, or captured console/page/request errors. No production discount data was mutated.
- `/admin/orders`, drag-enabled `/admin/collections`, and `/admin/discounts/:id/edit` local/live performance checks: plain orders page load made no initial `DateRangePickerWithPresets`, `react-day-picker`, `BulkShipDialog`, `DeleteOrderDialog`, `OrderItemsPopover`, `FraudCheckIndicator`, `SortableDataTableContent`, or `sortable.esm` module request. Local collections loaded `SortableDataTableContent.tsx` plus `@dnd-kit` deps on demand; live collections loaded `SortableDataTableContent-BdEuf433.js` and `sortable.esm-CsHTWzZ-.js` only for the sortable route. Local discount edit checks covered all three discount types and loaded only the matching form chunk; live discount edit checked the existing `amount_off_order` discount and loaded only `AmountOffOrderForm`.
