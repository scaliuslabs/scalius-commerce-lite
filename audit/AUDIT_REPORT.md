...
- Full `pnpm deploy`: latest row interaction lazy-loading slice redeployed API `05e91c24-b760-4019-a402-6a0cff4f2b35`, admin `4d9712cf-2a20-4db1-9061-1dee6e2a30e2`, and storefront `17df385d-d8c8-4385-b2d1-c0f570cc1db0`.
- Live HTTP and browser checks: API setup, demo sign-in through `POST /api/auth/sign-in/email`, authenticated `/admin/settings/cache`, authenticated `/admin/orders`, storefront `/`, storefront `/search`, and direct storefront purge-cache GET checks returned successfully after redeploy with no error-boundary page, stuck loading state, or new captured console/runtime errors.
- `/admin/orders` local and live performance checks: plain page load made no initial `DateRangePickerWithPresets`, `react-day-picker`, `BulkShipDialog`, `OrderItemsPopover`, or `FraudCheckIndicator` module request; first date-picker, item-popover, fraud-check, and selected-order bulk-ship interactions loaded their real modules on demand locally with no console/page errors; live first-click row checks loaded `OrderItemsPopover-TGEk5ypy.js` and `FraudCheckIndicator-BGNu5eia.js` only after interaction.
