...
- Latest local performance smoke covered `/admin/orders`: plain page load made no initial `DateRangePickerWithPresets`, `react-day-picker`, or `BulkShipDialog` module request; first date-picker click loaded the picker/calendar and showed `Quick select`; selected-order bulk ship loaded the dialog on demand with no console/page errors.
