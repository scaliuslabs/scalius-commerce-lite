...
- Latest OPS-006/PERF-012 API deploy is `da0da14c-9c9e-4f00-9595-1f1a26fa07b6`; live read-only checks passed for API health, `/readyz` (`ready`, D1 `1217ms`), checkout config (`success: true`, active default `cod`, checkout mode `all`, partial payment off), and OpenAPI contract smoke (`269` paths with payment-session and checkout-config routes present).
