...
- Latest OPS-006 local post-sale smoke uses `scripts/dev-postsale.mjs` against disposable Wrangler state only. It verifies fixture seeding, cart validation, COD order creation, idempotent replay, receipt read, support-request creation, bounded COD order load (`10/10` `201`, p95 `94.3ms`), and OTP fail-closed readiness with zero pending challenges when delivery credentials are dummy/unconfigured. Production mutation smokes remain disallowed; live checks stay read-only.
- Latest OPS-006 API deploy is `3ad9fbfd-ff19-4048-b622-d2b650b10f74`; live read-only checks passed for API health, `/readyz` (`ready`, D1 `131ms`), and checkout config (`success: true`, active default `cod`, checkout mode `all`).
