...
- `pnpm test` currently passes: 320 files and 2154 tests.
...
- Latest API runtime deploy is `a410bbc6-0eb7-4b69-8cd5-3e8b2d7208c1` from the PERF-011 public search/cache-cost policy slice. Cloudflare reported Worker startup time `113ms`; live read-only checks passed for API health, `/readyz` (`ready`, all checks ok), punctuation-only global search returning empty, excessive/empty global and product/category list limits returning `400`, public listing/search page cap `1000` returning `400` beyond the cap, normalized global search preserving canonical query text, whitespace-only `/products/search` behaving like blank search instead of empty invalid FTS, punctuation-only product/category listing returning empty instead of broad catalog reads, punctuation-only attribute filter search returning no filters, and OpenAPI exposing search limit `50`, product list limit `100`, public page cap `1000`, and `/products/search` validation errors. The previous PERF-010 deploy `f9cdb388-9e46-4e52-890e-0db1752dcea4` remains the latest remote D1 product-listing plan proof for the storefront listing indexes, and the earlier OPS-006/PERF-012 deploy `da0da14c-9c9e-4f00-9595-1f1a26fa07b6` remains the latest disposable local post-sale smoke evidence.
