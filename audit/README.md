...
- `pnpm test` currently passes: 318 files and 2139 tests.
...
- Latest API runtime deploy is `f9cdb388-9e46-4e52-890e-0db1752dcea4` from the PERF-010 storefront listing index slice. Cloudflare reported Worker startup time `124ms`; live read-only checks passed for API health, `/readyz` (`ready`, total `324ms`, D1 `127ms`), default product listing, `brand=Apple` product listing, storefront `/search?sortBy=newest`, and storefront `/search?brand=Apple`. Remote D1 plans use `products_public_newest_idx` for global newest, `products_public_category_newest_idx` for category newest, and `product_attribute_values_attr_value_product_idx` for single attribute filters without temp grouping. The earlier OPS-006/PERF-012 deploy `da0da14c-9c9e-4f00-9595-1f1a26fa07b6` remains the latest disposable local post-sale smoke evidence.
