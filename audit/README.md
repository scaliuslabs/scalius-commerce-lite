...
- `pnpm test` currently passes: 317 files and 2137 tests.
- `pnpm --filter @scalius/database check:migrations` passes: 75 SQL files, 75 journal entries, 32 snapshots, and 43 allowed manual snapshot gaps.
...
- Latest API runtime deploy is `032d668a-d918-4ecf-9917-0bf031d689c8` from the OPS-008 handler-graph startup slice. Cloudflare reported Worker startup time `127ms`; live read-only checks passed for API health, `/readyz` (`ready`, total `327ms`, D1 `136ms`), checkout config (`success: true`, active default `cod`, checkout mode `all`, partial payment off), and OpenAPI contract smoke (`269` paths with payment-session and checkout-config routes present). The earlier OPS-006/PERF-012 deploy `da0da14c-9c9e-4f00-9595-1f1a26fa07b6` remains the latest disposable local post-sale smoke evidence.
