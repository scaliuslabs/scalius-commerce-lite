...
- `pnpm test` currently passes: 128 files and 803 tests.
...
- Latest full `pnpm deploy` passed after the media/widget lazy-loading and `CACHE-013` hero-slider purge-resilience slice, redeploying API `9988d1c2-6058-45f0-b691-acf56652ad60`, admin `166ff295-428e-41c3-b318-41dd296f08eb`, and storefront `f886fcbf-7f1e-45b6-9701-d51c0e3a5961`.
- Live checks after the latest admin deploy covered API health, unauthenticated dashboard redirect, storefront `/`, demo sign-in through `POST /api/auth/sign-in/email`, and authenticated `/admin/discounts/disc_IcgNztBsUGqxYDHzqDtm1/edit` for the existing `amount_off_order` discount `OFFER250`. No checked page showed concrete error-boundary text or captured console/page/request errors after the final deploy. No production discount data was mutated.
- Latest local and live performance smokes covered `/admin/orders`, drag-enabled `/admin/collections`, discount editing, media pickers, and widget create. Plain orders page load made no initial `DateRangePickerWithPresets`, `react-day-picker`, `BulkShipDialog`, `DeleteOrderDialog`, `OrderItemsPopover`, `FraudCheckIndicator`, `SortableDataTableContent`, or `sortable.esm` module request. Local drag-enabled collections loaded `SortableDataTableContent.tsx` plus `@dnd-kit` deps on demand; live drag-enabled collections loaded `SortableDataTableContent-BdEuf433.js` and `sortable.esm-CsHTWzZ-.js` only on that route. Local discount edit smoke covered `amount_off_order`, `free_shipping`, and `amount_off_products`; each loaded only its matching form, and product/collection selectors loaded only for `amount_off_products`. Live discount edit smoke covered the existing `amount_off_order` discount and loaded only `AmountOffOrderForm`. Media picker smokes covered `/admin/products/new`, `/admin/media`, and `/admin/settings/hero-sliders`; widget smokes covered paste, preview iframe, and copy-prompt without saving production data.
