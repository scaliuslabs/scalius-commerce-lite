...
- Latest full `pnpm deploy` passed after the `PERF-003` orders interaction lazy-loading slice and redeployed API `9414dd72-3b67-42cd-9f3d-914482a76579`, admin `797cf037-7158-45a3-81bd-7216810517da`, and storefront `427a51c6-7a2e-44f9-afa7-020b346217c5`.
- Live checks covered API setup, authenticated `/admin/settings/cache`, authenticated `/admin/orders`, storefront `/`, storefront `/search`, and direct storefront purge-cache GET checks. No checked page showed concrete error-boundary text or new console/runtime errors after the final deploy.
- Latest local performance smoke covered `/admin/orders`: plain page load made no initial `DateRangePickerWithPresets`, `react-day-picker`, `BulkShipDialog`, `OrderItemsPopover`, or `FraudCheckIndicator` module request; first date-picker, item-popover, fraud-check, and selected-order bulk-ship interactions loaded their real modules on demand with no console/page errors. The latest deployed/live performance smoke currently covers the date-picker and bulk-ship boundaries; record row-popover/fraud live evidence after the next deploy.
