...
3. The code has a split-brain async/sync order story. `createStorefrontOrder()` still says it returns a payload ready for `ORDER_INGEST_QUEUE`: `packages/core/src/modules/orders/orders.storefront.ts:26-30`, and API Wrangler declares an `ORDER_INGEST_QUEUE`: `apps/api/wrangler.jsonc:64-112`. The public create route now commits with `commitStorefrontOrderPayload()` synchronously before scheduling checkout status/receipt hints; tests assert the queue is not used. Storefront still has a 202 polling branch for duplicate in-flight/older async order responses: `apps/storefront/src/lib/api/orders.ts`.

4. KV is used for checkout recovery hints, not checkout authority. The create/status paths write `checkout_status:{token}` and `order_receipt:{sha256(token)}` after or while repairing from D1 authority. Payment session and receipt reads validate through D1 first, with KV only as a cache: `apps/api/src/utils/order-receipt-token.ts`. Cloudflare documents KV as eventually consistent and not ideal for atomic/read-write coordination, so D1 must remain the authority.
