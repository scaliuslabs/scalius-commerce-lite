...
- Rights-safe media generation is in progress through the private offline staging boundary. The 79 newly generated records are now provenance-approved, visually reviewed, and normalized: 36 complete Footwear records for Vale, Monsoon, Nila, Padma, Dune, Metro, Rove, Ridge, and Aster; 39 complete Home & Living records for Mira, Soma, Noor, Jute, Luma, Kori, Dhara, Arka, and Terra; and the complete four-image Kansa Serve Bowl set with a contained primary, two construction details, and an editorial lifestyle view. They preserve exact option appearance and distinct primary/variant/detail/lifestyle roles. The partial run truthfully exits 2 with `ready: false`, `staged: 79`, `missing-source-record: 158`, and zero manifest errors; the focused registration/provenance/staging gate passes 3 files and 16 tests. Rider's four existing records remain outside this count and deliberately unapproved until the retained-Media exporter proves exact live bindings and a human can supply honest ownership evidence; Halo's retained primary/video and derived poster/detail relationship remain under the same fail-closed evidence boundary. No downloaded hash alone is rights evidence, and no staged asset has been uploaded or made public yet.
