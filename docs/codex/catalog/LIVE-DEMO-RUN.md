...
## Rich-demo readiness milestone (local/offline)

The replacement rich-demo dataset is fully prepared but has **not** been uploaded or applied to production. The private ignored asset boundary is ready at 237 of 237 staged records: 50 products, 5 categories, 177 SKUs, 49 ordered additional sections, 18 offers, and 3 hero stories. Every generated asset and every retained-media replacement was visually reviewed, including all 3 product videos. `pnpm demo:store --plan` passed, `pnpm demo:store --compile` produced 122 commands, and the complete demo-store suite passed 13 files / 98 tests after removal of the obsolete exact-eight retained export path.

The first authenticated Media upload attempt failed before any remote write. Admin sign-in/readiness reached an unavailable D1 authority, and a direct read-only Wrangler query returned Cloudflare error `7429` (`D1 DB is overloaded. Requests queued for too long.`). No private Media upload journal and no apply-readiness output exist, so there is no partial upload or ambiguous apply state to reconcile.
...
2. Upload all staged Media through the authenticated resumable bridge:
...
- The checked-in rich-store plan passes `pnpm demo:store --plan`: 5 categories, 50 products, 177 SKUs, 46 optioned plus 4 simple products, 237 media intents, 49 ordered additional sections, 5 collections, 18 offers, and 3 hero stories. `pnpm demo:store --compile` also passes and emits 122 ordered commands. The authenticated executor and rights-safe assets are complete locally; production writes remain blocked only until D1 readiness recovers and the upload/diff/guarded-apply sequence above succeeds.
- `pnpm demo:store --diff` is also implemented as a write-disabled authenticated reconciliation: hidden interactive credentials, bounded reads, exact slug/retained-ID matching, private ignored evidence exports, safe allowlisted JSONL resume metadata, and best-effort session cleanup. Apply remains fail-closed and has not been invoked because the D1 outage prevented creation of the required Media readiness file; credentials and confirmations must enter only through its interactive prompts.
...
- A separate Media-only bridge requires all 237 staged records before it can upload sequentially. It keeps a 0600 resume journal, verifies remote hashes and dimensions, applies the exact retained Rider/Halo replacement authority, records poster relationships, and has no product/publication write surface. The failed pre-write attempt produced neither the journal nor `.wrangler/demo-store-assets/apply-readiness.json`.
