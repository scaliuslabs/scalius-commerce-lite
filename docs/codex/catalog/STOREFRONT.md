...
8. **Resolved in the feed/UCP authority slice: UCP pagination is based on the feed-eligible projection.** The feed query applies active public-product, buyer-resolvable SKU, and usable primary-discovery-image eligibility before counting and pagination; UCP maps those rows and enforces the exact protocol version.
