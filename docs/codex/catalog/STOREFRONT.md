...
- Collection membership follows canonical `config.source`: manual collections use only the saved ordered product IDs; dynamic collections use only selected categories and automatically admit newly eligible products. Runtime legacy inference is intentionally absent. Public list/detail, sitemap, canonical, and structured-data visibility require an active non-deleted collection; homepage resolution additionally requires explicit `config.showOnHomepage`. Homepage placement never controls the public collection page.
