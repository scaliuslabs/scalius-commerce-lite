...
- Open cache dependency: product/category writes invalidate homepage projections but the current product/category invalidation groups do not clear `api:collections:*` or `collection_by_id_*`. Collection detail pages can therefore lag membership, price, media, or availability truth until TTL; fix this in the owning cross-domain cache slice and add stock/product/category mutation regressions.
