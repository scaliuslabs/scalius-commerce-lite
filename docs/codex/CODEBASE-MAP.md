...
- Admin in-tab idle navigation relies on `apps/admin-v2/src/lib/admin-route-context.ts`: verified route context is fresh for 1 minute, stale-while-revalidated until a 4-hour hard cap, and explicitly cleared through `refreshAdminRouteContext(router)` after profile/security/session/permission changes. Auth/setup/2FA success paths navigate through TanStack Router instead of hard reloading. The auth guard caches only positive admin-exists reads briefly, uses `SELECT 1 ... LIMIT 1` with one in-flight read per Worker isolate, and relies on `user_role_idx` / `user_super_admin_idx`; no-cookie auth guards skip Better Auth binding initialization/session reads after setup detection. Authenticated hot guards resolve the Better Auth session cookie through the narrow direct D1 helper in `apps/admin-v2/src/lib/admin-session.server.ts`, keeping `auth.server.ts` and Better Auth out of the normal `/admin` guard closure. RBAC loading reuses that fresh `user.isSuperAdmin` value to return before Cloudflare/database/core RBAC dynamic imports for known super-admins, and non-super-admin auto-seed uses a versioned six-hour `CACHE` KV marker from `packages/core/src/auth/rbac/auto-seed.ts` to avoid repeating the expensive seed-current D1 batch in fresh isolates. Cold SSR remains authoritative for authenticated requests and still performs real auth/session/RBAC reads, but the admin route shell keeps its first-load graph small: `AdminNav` and `admin-access.ts` use local permission-name mirrors from `apps/admin-v2/src/lib/admin-permissions.ts` guarded by parity tests against core RBAC, `AppSidebar` lazy-loads `StorefrontFooterLink` after idle so the storefront URL query/server-function graph stays out of the initial shell, `AdminHeader` defers `UserMenu` so dropdown/avatar/auth-client code loads after idle, and `sidebar.tsx` lazy-loads the mobile Sheet/Dialog stack from `sidebar-mobile-sheet.tsx` only after the mobile drawer first opens. The admin dashboard home intentionally does not block SSR or client route entry on metrics. It fetches `/api/v1/admin/dashboard/home-summary` after hydration/navigation, keeps the full `/dashboard/summary` contract for legacy callers, imports summary/activity options through `api-query-options/dashboard-home.ts`, lazy-loads `DashboardStats`, `RecentOrders`, and `WelcomeBanner`, and keeps currency reads isolated in `api-query-options/currency.ts` so homepage/order chunks do not import the broad settings graph. Admin list route `validateSearch` handlers must use the dependency-free `createListSearchValidator()` and primitive normalizers from `apps/admin-v2/src/lib/list-helpers.tsx`; Zod belongs in form/API validation, not the eager route tree.
- Admin hero-slider settings keep the default route path light: `HeroSliderContainer` lazy-loads `SliderTab`, `SliderTab` lazy-loads `SortableSlidesEditor` only after Reorder, and `@dnd-kit/*`/portal overlay code stays in that sortable chunk. The media-manager index barrel is picker-only and must not export `MediaManagerPage`; the standalone `/admin/media` route imports that page directly.
- Admin notification settings may use shared order notification constants from `@scalius/core/modules/notifications/notification-types`, but must not import the broad `@scalius/core/modules/notifications` barrel because that barrel re-exports server-only services, outbox, receipt, database, and provider code into client chunks.
- Inventory expiry is an orphan-reservation cleanup path, not stale-order cancellation. `releaseExpiredReservations()` processes a bounded batch (`50` by default, max `200`), returns `hasMore`, and claims each expiry release with a deterministic movement id plus variant counter update inside D1 `safeBatch()`.
- Catalog, CMS, and attribute writes must keep affected storefront HTML warm after freshness bumps: `invalidateCatalogCaches()` warms canonical `/search` for product/category/discount writes; product create/update/delete/restore/permanent-delete and variant routes also include affected `/products/{slug}` before category/listing paths; category/product routes add affected `/categories/{slug}` paths when cheaply known. CMS page create/update/bulk-publish/restore/single-restore resolves post-write public `/{slug}` paths through `invalidateApiAndScheduleStorefrontGroups(["pages", "layout"], c, { htmlPaths })`, while delete/unpublish skips rewarming now-hidden pages. Attribute metadata/value writes pass `{ htmlPaths: ["/search"] }` through `invalidateApiAndScheduleStorefrontGroups()`. These exact paths ride the storefront prefix purge payload, are canonicalized/capped, and are warmed after the global version moves.
