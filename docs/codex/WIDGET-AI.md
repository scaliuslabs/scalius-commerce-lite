...
- No-context generations add an explicit factuality gate and the API rejects unsupported commerce claims during normalization. Delivery/shipping, guarantees, reviews, scarcity, price/discount, absolute URL, and product-performance claims require selected product/category/collection context instead of model invention.
- If a no-context model response still cannot be repaired into a safe artifact, the API returns a deterministic generic discovery widget instead of failing the merchant action.
