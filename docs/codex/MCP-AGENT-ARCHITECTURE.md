...
- The first MCP-powered admin chat bridge lives in the API, not the browser: `apps/api` uses the `AGENT` service binding, initializes `http://agent.internal/mcp/admin`, calls `admin_navigation_context` on every chat, and conditionally calls `admin_product_search` plus `admin_product_copy_context` for product-description/copywriting intents. It forwards only the dashboard cookie plus sanitized User-Agent, strips bearer auth, compacts MCP output into bounded prompt context, redacts phone/email/token-like values, and returns optional click-confirmed `navigate` actions. The model never receives credentials, raw MCP output, dynamic detail URLs, price/SKU/stock/barcode/image facts, or mutation authority, and the dashboard sanitizes the action path again before TanStack navigation.
...
The first visible Admin Assistant slice is intentionally smaller than full tool-use: the dashboard exposes a movable assistant bubble, opens a compact floating chat panel by default, and lets the merchant switch that panel into a right sidebar when they want persistent workspace space. It must not use a blocking modal/sheet overlay for ordinary chat. The chat sends bounded conversation history plus the existing sanitized page-state snapshot through an admin server function, renders simple assistant markdown as UI instead of raw `**bold**` or backtick text, and the API calls the configured `adminChat` model profile. It is stateless, no-store, and read-only. The MCP-powered runtime context it may use today is the API-owned navigation catalog plus bounded product search/copy context for product-description assistance. It still has no page tools, Code Mode, direct domain reads outside explicitly allowlisted read tools, or mutation authority.

## Next Capability Slices

Do not keep iterating on the product-copy read slice before these higher-value gaps are addressed.

1. Admin visible page-action protocol: `apps/admin-v2/src/components/admin/assistant/**`, the admin AI server function, and `apps/api/src/routes/admin/ai.ts` should agree on browser-owned, click-confirmed actions for focusing a registered surface, applying a draft, saving a registered form, selecting visible rows, and clearing selection. The model proposes actions; the browser executes only actions backed by registered handlers.
2. Registered product-form actions: extend `ProductForm.tsx` and `product-form/assistantSurface.ts` first for safe visible fields such as name/description. Keep price, SKU, stock, images, credentials, and hidden fields out until each has a dedicated design. Saving must be a separate user click and must reuse existing form validation/submit paths.
3. Visible table selection: add a small table-surface helper around the existing admin data-table code, then wire products first. It may expose visible row IDs and selected counts only; it must not read hidden rows, customer PII, or make API calls.
