...
- The first MCP-powered admin chat bridge lives in the API, not the browser: `apps/api` uses the Admin Agent service binding, calls `admin_navigation_context` on every chat, conditionally calls `admin_dashboard_summary` for authoritative aggregate questions, and conditionally calls `admin_product_search` plus `admin_product_copy_context` for product-description/copywriting intents. It forwards only the dashboard cookie plus sanitized User-Agent, strips bearer auth, compacts MCP output and page action metadata into bounded prompt context, redacts phone/email/token-like values, and returns optional catalog-derived `navigate` or registered page-action proposals. A direct, unambiguous current-turn command to one catalog-known route may count as confirmation only when the user wording agrees with the sanitized action label/path; suggestions and ambiguous requests remain click-confirmed. The model never receives credentials, raw MCP output, dynamic detail URLs, price/SKU/stock/barcode/image facts, hidden handlers, hidden field values, or mutation authority, and the dashboard sanitizes actions again before navigation or browser-owned execution.
...
The first visible Admin Assistant slice is intentionally smaller than full autonomous tool-use: the dashboard exposes a movable assistant bubble, opens a compact floating chat panel by default, and lets the merchant dock it as a true full-height left or right workspace sidebar. Desktop docks participate in the Admin shell flex layout and reflow the page instead of floating over it; compact/mobile docks use the viewport because there is no safe content width to preserve. It must not use a blocking modal/sheet overlay for ordinary desktop chat. The chat sends bounded conversation history plus the existing sanitized page-state snapshot through an admin server function, renders simple assistant markdown as UI instead of raw `**bold**` or backtick text, and the API calls the configured `adminChat` model profile. Merchant-facing assistant text must strip provider tool-call/function-call artifacts and fall back to safe guidance when a model returns raw tool syntax. Its runtime context is restricted to explicitly allowlisted Admin MCP reads, including authoritative dashboard aggregates and bounded product search/copy context. Its page-tool protocol is browser-owned and registered: direct matching navigation commands may execute as described above, while draft/form/table proposals remain click-confirmed and page-revalidated. It still has no Code Mode, hidden domain mutations, dialog tools, or unallowlisted domain reads.
