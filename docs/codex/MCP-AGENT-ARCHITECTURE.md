...
The first visible Admin Assistant slice is intentionally smaller than full autonomous tool-use: the dashboard exposes a movable assistant bubble, opens a compact floating chat panel by default, and lets the merchant switch that panel into a right sidebar when they want persistent workspace space. It must not use a blocking modal/sheet overlay for ordinary chat. The chat sends bounded conversation history plus the existing sanitized page-state snapshot through an admin server function, renders simple assistant markdown as UI instead of raw `**bold**` or backtick text, and the API calls the configured `adminChat` model profile. It is stateless and no-store. The MCP-powered runtime context it may use today is the API-owned navigation catalog plus bounded product search/copy context for product-description assistance. Its first page-tool protocol is browser-owned, registered, and click-confirmed: product create/edit forms can expose safe name/description focus, draft application, and form-save proposals that the page revalidates before execution. It still has no Code Mode, hidden domain mutations, table/dialog tools, storefront model chat, or direct domain reads outside explicitly allowlisted read tools.

## Next Capability Slices

Do not keep iterating on the product-copy read slice before these higher-value gaps are addressed.

1. Complete the Admin visible page-action protocol beyond the first product-form slice: `apps/admin-v2/src/components/admin/assistant/**`, the admin AI server function, and `apps/api/src/routes/admin/ai.ts` now agree on browser-owned, click-confirmed actions for focusing a registered surface, applying a draft, saving a registered form, selecting visible rows, and clearing selection. The remaining work is adding table/dialog registrations and release browser smokes. The model proposes actions; the browser executes only actions backed by registered handlers.
