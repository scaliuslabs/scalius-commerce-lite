...
1. Complete the Admin visible page-action protocol beyond the first product-form slice: `apps/admin-v2/src/components/admin/assistant/**`, the admin AI server function, and `apps/api/src/routes/admin/ai.ts` now agree on browser-owned, click-confirmed actions for focusing a registered surface, applying a draft, saving a registered form, selecting visible rows, and clearing selection. The products table is wired; remaining work is dialog registration, storefront model chat, and release browser smokes. The model proposes actions; the browser executes only actions backed by registered handlers.
