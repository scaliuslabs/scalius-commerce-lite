...
- The first implemented admin slice is intentionally tiny: `apps/agent` accepts `/mcp/admin` only on the internal service-binding host `http://agent.internal/mcp/admin`, `apps/admin-v2` proxies it from `/api/assistant/mcp`, and the only tools are read-only context helpers: `admin_session_context` and `admin_navigation_context`.
