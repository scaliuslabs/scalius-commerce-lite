...
ASSUME THE CODEBASE IS BROKEN UNTIL PROVEN OTHERWISE. The previous session introduced a massive migration (loaders from DB to API) and despite multiple rounds of fixes, bugs kept appearing in production that weren't caught locally. There could be hundreds of broken implementations, subtle data shape mismatches, missing fields, wrong envelope parsing, silent failures, or components that render but show wrong data. Every time you read a file and something looks off — a fetch call parsing json.data when the proxy already unwrapped it, a component expecting a field the API doesn't return, a form submission targeting a wrong endpoint, an error handler swallowing errors silently — TEST IT. Don't assume it works just because it doesn't crash. Start the dev server, open Chrome, navigate to that page, click the button, fill the form, check the console. If you find an issue, fix it locally, verify it works, then tell me to test it in production too before moving on.

Before you start making ANY changes, start the dev servers (pnpm dev), open Chrome, and verify every single admin page loads correctly with no console errors. Click into every form, every edit page, every settings tab. Check that data actually renders — not just that the page loads. If anything is broken from the previous session, fix it FIRST before starting new work. The previous session left most pages untested and bugs kept appearing in production every time I tried to use a feature. Don't repeat that mistake.
