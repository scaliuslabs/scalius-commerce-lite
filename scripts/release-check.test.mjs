...
function adminAnalyticsSummaryMcpResult(extra = {}) {
  const summary = {
    source: {
  ...
  const body = { adminAnalyticsSummary: summary };
  return {
    content: [{ type: "text", text: JSON.stringify(body) }],
    structuredContent: body,
  };
...
function adminPageSearchMcpResult(extra = {}) {
  const body = {
    adminPageSearch: {
      query: "test",
      page: 1,
...
function adminMediaSearchMcpResult(extra = {}) {
  const body = {
    adminMediaSearch: {
      query: "test",
      page: 1,
...
function adminInventoryLookupMcpResult(extra = {}) {
  const body = {
    ...
    if (parsed.pathname === ADMIN_ASSISTANT_MCP_PATH) {
      ...
      if (body.method === "tools/call") {
        ...
        if (body.params?.name === "admin_navigation_context") {
          ...
          result = typeof analyticsSummaryToolResult === "function"
            ? analyticsSummaryToolResult(body)
        ...
        } else if (body.params?.name === "admin_page_search") {
          expect(body.params).toEqual({
          ...
          result = typeof pageSearchToolResult === "function"
            ? pageSearchToolResult(body)
            : pageSearchToolResult;
          cacheControl = pageSearchToolCacheControl;
        } else if (body.params?.name === "admin_media_search") {
          expect(body.params).toEqual({
          ...
          result = typeof mediaSearchToolResult === "function"
            ? mediaSearchToolResult(body)
          ...
          result = typeof inventoryLookupToolResult === "function"
            ? inventoryLookupToolResult(body)
...
describe("release-check local evaluators", () => {
  ...
  it("fails authenticated Admin MCP smoke when analytics summary lacks no-store, errors, malformed structure, unsafe limits, or leaks sensitive data", async () => {
    await expect(smokeAdminMcpAuthenticated({
      ...
      password: "correct-password",
      fetchImpl: authenticatedAdminMcpSmokeFetch({
        analyticsSummaryToolCacheControl: "public, max-age=60",
      }),
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_analytics_summary Cache-Control must include no-store");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      fetchImpl: authenticatedAdminMcpSmokeFetch({
        analyticsSummaryToolResult: {
          ...
          structuredContent: {
            error: {
              code: "admin_analytics_summary_failed",
              status: 500,
            },
          },
        },
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_analytics_summary returned an MCP tool error");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_analytics_summary must return at least one content block");
    ...
    await expect(smokeAdminMcpAuthenticated({
          ...
          structuredContent: {
            adminAnalyticsSummary: {
              source: {
                ...
                version: "admin-analytics-summary:v0",
              },
              summary: { totalProviders: "6" },
              providers: {},
              limits: {
                includesScriptConfig: false,
                includesAnalyticsSnippets: false,
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_analytics_summary source.path must be /api/v1/admin/analytics/health");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_analytics_summary limits must explicitly disable includesCredentials");
    ...
    await expect(smokeAdminMcpAuthenticated({
          ...
          providers: [
            {
              ...
              browser: {
                ...
                blockedScriptCount: 1,
                issueCount: 1,
                issues: ["Access token leaked"],
              },
              serverSide: {
                status: "blocked",
                configured: false,
                label: "Server blocked",
                message: "pixel id 123456",
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("summary must not leak analytics config, snippets, custom code");
  });
  ...
  it("fails authenticated Admin MCP smoke when page search lacks no-store, errors, or empty content", async () => {
    await expect(smokeAdminMcpAuthenticated({
      dashboardUrl: "https://dashboard.example.test",
      email: "admin@example.test",
      password: "correct-password",
      fetchImpl: authenticatedAdminMcpSmokeFetch({
        pageSearchToolCacheControl: "public, max-age=60",
      }),
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_page_search Cache-Control must include no-store");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_page_search returned an MCP tool error");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_page_search must return at least one content block");
  });

  it("fails authenticated Admin MCP smoke when media search lacks no-store, errors, or empty content", async () => {
    await expect(smokeAdminMcpAuthenticated({
      dashboardUrl: "https://dashboard.example.test",
      email: "admin@example.test",
      password: "correct-password",
      fetchImpl: authenticatedAdminMcpSmokeFetch({
        mediaSearchToolCacheControl: "public, max-age=60",
      }),
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_media_search Cache-Control must include no-store");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      fetchImpl: authenticatedAdminMcpSmokeFetch({
        mediaSearchToolResult: {
          ...
          structuredContent: {
            error: {
              code: "admin_media_search_failed",
              status: 500,
            },
          },
        },
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_media_search returned an MCP tool error");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_media_search must return at least one content block");
  });
  ...
  it("fails authenticated Admin MCP smoke when inventory lookup lacks no-store, errors, or empty content", async () => {
    await expect(smokeAdminMcpAuthenticated({
      ...
      password: "correct-password",
      fetchImpl: authenticatedAdminMcpSmokeFetch({
        inventoryLookupToolCacheControl: "public, max-age=60",
      }),
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_inventory_lookup Cache-Control must include no-store");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      fetchImpl: authenticatedAdminMcpSmokeFetch({
        inventoryLookupToolResult: {
          ...
          structuredContent: {
            error: {
              code: "admin_inventory_lookup_failed",
              status: 500,
            },
          },
        },
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_inventory_lookup returned an MCP tool error");
    ...
    await expect(smokeAdminMcpAuthenticated({
      ...
      timeoutMs: 5_000,
      logger: null,
    })).rejects.toThrow("admin_inventory_lookup must return at least one content block");
  });
