...
Rider and Halo must explicitly account for every existing ready Media identity.
When the current bytes have approved provenance, reuse the exact identity with
an explicit mapping on the corresponding source record:
...
Every current ready direct or poster association on a retained product must be
covered exactly once by either `remoteReuse` or `retainedReplacement`.
