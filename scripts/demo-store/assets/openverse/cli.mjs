...
import { discoverOpenverseCandidates } from "./discovery.mjs";

const here = path.dirname(fileURLToPath(import.meta.url));
let planPath = path.join(here, "queries.json");
let outputPath = path.resolve(".wrangler/demo-store-assets/openverse-review.json");
...
if (plan.schemaVersion !== 1) throw new Error("Openverse query plan must use schemaVersion 1");
const queue = await discoverOpenverseCandidates({ queries: plan.queries });
